/*HTC start Tom Lin 2011/12/19 Sony IMX091 8M driver */
...
#define IMX091_READ_NORMAL_MODE 0x0000	/* without mirror/flip */
#define IMX091_READ_MIRROR 0x0001			/* with mirror */
#define IMX091_READ_FLIP 0x0002			/* with flip */
#define IMX091_READ_MIRROR_FLIP 0x0003/* with mirror/flip */
...
static struct msm_sensor_output_info_t imx091_dimensions[] = {
	{/*full size 3280 x 2464 at 20 fps*/
		...
		.vt_pixel_clk = 225600000,/*24Mhzx2(PreDivider)x47(PLL)/1(PostDivider)=564(Pixel CLK) /10(vt_pixel_clk_divider)* 1(vt_sys_clk_div) = 56.4MHZx4(lanes)*/
		.op_pixel_clk = 225600000,
		.binning_factor = 1,
		.x_addr_start = 0x08, //8
		.y_addr_start = 0x30, //48
		.x_addr_end = 0x1077, //4215
		.y_addr_end = 0xC5F, //3167
		.x_even_inc = 1,
		.x_odd_inc = 1,
		.y_even_inc = 1,
		.y_odd_inc = 1,
...
static struct msm_sensor_exp_gain_info_t imx091_exp_gain_info = {
	...
	.vert_offset = 5, /* sony imx091 sensor limitation */ /* HTC Tom 20120210 - Fix black screen */
...
int32_t imx091_power_up(struct msm_sensor_ctrl_t *s_ctrl)//(const struct msm_camera_sensor_info *sdata)
{
	int rc;
...
int32_t imx091_power_down(struct msm_sensor_ctrl_t *s_ctrl)//(const struct msm_camera_sensor_info *sdata)
{
	int rc;
	struct msm_camera_sensor_info *sdata = NULL;
	pr_info("%s\n", __func__);
...
/*HTC end Tom change back to read word*/
	...
	cdata->cfg.fuse.fuse_id_word4 =
		...
		(OTP[9]);
/*
	pr_info("imx091: fuse->fuse_id_word1:%d\n",
		cdata->cfg.fuse.fuse_id_word1);
	pr_info("imx091: fuse->fuse_id_word2:%d\n",
		cdata->cfg.fuse.fuse_id_word2);
	pr_info("imx091: fuse->fuse_id_word3:0x%08x\n",
		cdata->cfg.fuse.fuse_id_word3);
	pr_info("imx091: fuse->fuse_id_word4:0x%08x\n",
		cdata->cfg.fuse.fuse_id_word4);
*/
...
static int imx091_read_VCM_driver_IC_info(	struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	//Select byte. Driver ID at 2th byte
	page = 2;
...
/*HTC end Tom Lin 2011/12/19 Sony IMX091 8M driver*/
