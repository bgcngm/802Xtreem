...
bool_t	Yushan_Init_LDO(bool_t	bUseExternalLDO)
{
	...
	if (bSpiReadData == 1) {
		...
		fStatus &= Yushan_WaitForInterruptEvent(EVENT_LDO_STABLE, TIME_100MS/*TIME_20MS*/);
...
bool_t Yushan_Init_Clocks(Yushan_Init_Struct_t *sInitStruct, Yushan_SystemStatus_t *sSystemStatus, uint32_t	*udwIntrMask)
{
	...
	*  wait till Pll gets lock/stable. */
	fStatus &= Yushan_WaitForInterruptEvent(EVENT_PLL_STABLE, TIME_100MS/*TIME_20MS*/);
...
bool_t	Yushan_Init_Dxo(Yushan_Init_Dxo_Struct_t *sDxoStruct, bool_t fBypassDxoUpload)
{
	...
	*  to know the actual physical address to access. */
	...
	fStatus &= Yushan_WaitForInterruptEvent2 (EVENT_DOP7_BOOT, TIME_100MS/*TIME_10MS*/);
	if (!fStatus) {
	...
	/* Wait for the End of Boot interrupts. This indicates the completion of Boot command */
	fStatus &= Yushan_WaitForInterruptEvent (EVENT_DPP_BOOT, TIME_100MS/*TIME_10MS*/);
	if (!fStatus) {
	...
	/* Wait for the End of Boot interrupts. This indicates the completion of Boot command */
	fStatus &= Yushan_WaitForInterruptEvent (EVENT_PDP_BOOT, TIME_100MS/*TIME_10MS*/);
...
bool_t Yushan_Update_Commit(uint8_t  bPdpMode, uint8_t  bDppMode, uint8_t  bDopMode)
{
	...
	/* execution command */
	fStatus = Yushan_WaitForInterruptEvent(EVENT_PDP_EOF_EXECCMD, TIME_100MS/*TIME_10MS*/);
	if (!fStatus) {
	...
	fStatus = Yushan_WaitForInterruptEvent2(EVENT_DOP7_EOF_EXECCMD, TIME_100MS/*TIME_10MS*/);
	if (!fStatus) {
	...
	fStatus = Yushan_WaitForInterruptEvent(EVENT_DPP_EOF_EXECCMD, TIME_100MS/*TIME_10MS*/);
	if (!fStatus) {
...
// HTC_START chenen 20121105 In case stream has not been started yet
