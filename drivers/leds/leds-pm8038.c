...
#define WLED_DIG_MOD_GEN_SET		0x10	/* LED PWM disabled, LED1 PWM active*/
#define WLED_CS_OUT_SET			0x02	/* LED2 disabled, LED1 active */
...
#define WLED_FSW_CTRL_SET		0x00	/* Boot switch freq = 800kHz */
#define WLED_MOD_CLK_CTRL_SET		0x06	/* Digital modulator input clock freq = 1.2 MHz */
#define WLED_CNTRL_14_RESERVED_MASK	0x0F
#define WLED_CNTRL_14_RESERVED_SET	0x03
#define WLED_CNTRL_15_RESERVED_SET	0x3F	/* QCT recommended value */
...
///HTC:
	if (led->id == PM8XXX_ID_WLED) {
		/* If select WLED_CABC, skip led_wled_set.
		   We only switch on/off WLED module. */
#ifdef CONFIG_BACKLIGHT_WLED_CABC
...
///HTC:
	/* These two registers are related to efficiency */
...
#ifdef CONFIG_BACKLIGHT_WLED_CABC
	/* Fix LED_duty_cycle to 100% and select LED_CABC as input source */
	val = 0x7F;
...
///HTC:
	...
	/* Turn off WLED in Power Test and offmode charging with animation */
	if (board_mfg_mode() == 4 ||
...
static int __devinit pm8xxx_led_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < pcore_data->num_leds; i++) {
		...
		/* Init the lut multiply the coefficient  */
