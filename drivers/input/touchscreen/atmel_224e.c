...
/* factor of ABS_MT_TOUCH_MAJOR and ABS_MT_PRESSURE */
#define TOUCH_MAJOR_FACTOR                      15
#define PRESSURE_FACTOR                         0x7f
				...
				struct atmel_finger_data *fdata, uint8_t press, uint8_t last, uint8_t report_type, uint8_t slot_num)
{
	...
	if (report_type == SYN_AND_REPORT_TYPE_B) {
			...
			if ( fdata->z > PRESSURE_FACTOR ) {
				input_report_abs(idev, ABS_MT_PRESSURE, PRESSURE_FACTOR);
			} else {
				input_report_abs(idev, ABS_MT_PRESSURE, fdata->z);
			}
			if ( fdata->w > ts->abs_width_max ) {
				input_report_abs(idev, ABS_MT_TOUCH_MAJOR, ts->abs_width_max*TOUCH_MAJOR_FACTOR);
			} else {
				input_report_abs(idev, ABS_MT_TOUCH_MAJOR, fdata->w*TOUCH_MAJOR_FACTOR);
			}
...
#if defined(CONFIG_TOUCHSCREEN_ATMEL_WLS)
	if (connect_status == 4 || ((connect_status != 4) && (ts->wlc_status))) {
		wlc_tp_status_handler_func(NULL, connect_status == 4 ? 1 : 0, NULL);
		if (connect_status == 4 || ((connect_status != CONNECT_TYPE_UNKNOWN) && (connect_status <= 0))) {
			return;
		}
	}
#endif
			 ...
			 const struct i2c_device_id *id)
{
	...
	if (pdata) {
		...
		if ((cable_connect_type > 0) || (cable_connect_type == CONNECT_TYPE_UNKNOWN))
			ts->status = CONNECTED;
#endif
	...
	input_set_abs_params(ts->input_dev, ABS_MT_TOUCH_MAJOR,
				ts->abs_width_min, ts->abs_width_max*TOUCH_MAJOR_FACTOR,
				0, 0);
	input_set_abs_params(ts->input_dev, ABS_MT_PRESSURE,
				ts->abs_pressure_min, PRESSURE_FACTOR,
				0, 0);
