...
static int hdcp_authentication_part1(void)
{
	...
	if (!is_part1_done) {
		...
		/* Do not read AN0 and AN1 before clock is set.
		 * If clock is not set yet, HDCP auth. will always fail
		 * due to wrong AN values.
		 * However, we let the auth. continue to pass the test.
		 * part1 will fail anyway and reauth will do the job.
		 */
		if (clock_not_set) {
			link0_an_0 = 0;
			link0_an_1 = 0;
