				  ...
				  struct mdp4_overlay_pipe *pipe)
{
	...
	if (pipe->flags & MDP_DEINTERLACE) {
		...
		/* HTC: Request higher clock for small VBP.
		 *      Then it can use writeback
		 */
			      ...
			      struct mdp4_overlay_pipe *plist)
{
	...
	for (i = 0; i < OVERLAY_PIPE_MAX; i++, pipe++) {
		...
		/* HTC Remind: Keep sync mdp4_overlay_blt_check from here for
		   any force blt cases */
		if (pipe->req_clk > mdp_max_clk) {
			if (pipe->mixer_num == MDP4_MIXER0)
				perf_req->use_ov0_blt = 1;
			if (pipe->mixer_num == MDP4_MIXER1) {
				/* MDP4.4 needs max clk (266Mhz) when req_clk > defined max clk in mixer 1
				 * It may solve mdp underrun in some corner cases, like camera enter/exit
				 * animation in external display.
				 */
		...
		if (pipe->mixer_num == MDP4_MIXER0) {
			/*Non-RGB format, set MDP bw to OVERLAY_PERF_LEVEL4 */
			switch (pipe->src_format) {
			case MDP_XRGB_8888:
			case MDP_ARGB_8888:
			case MDP_RGBA_8888:
 ...
 *  This helper function show current mdp perf request stat,
 *  which is used for underflow or performance debugging
 *  This may be called from any context. Criticall section should
	...
	if (img->flags & MDP_MEMORY_ID_TYPE_FB) {
		...
		if (MAJOR(file->f_dentry->d_inode->i_rdev) == FB_MAJOR) {
			...
			if (get_fb_phys_info(start, len, fb_num,
				...
				/* Media Link will use FB memory as source image
				   (MDP_MEMORY_ID_TYPE_FB), remove this warning.
