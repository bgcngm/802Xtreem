...
static struct vsycn_ctrl {
	...
	/* TODO:for Myst issue clarify, remove it later*/
	int temp_cnt;
...
void mdp4_dsi_cmd_vsync_ctrl(struct fb_info *info, int enable)
{
	...
	/* HTC: After suspended, always turn off vsync ctrl */
...
static void primary_rdptr_isr(int cndx)
{
	...
	if (vctrl->expire_tick) {
		...
		if ((vctrl->watchdog & 0x0f)==0) {
			/* HTC: show blocked processes to debug hang problems */
			...
			struct mdp4_overlay_pipe *pipe)
{
	...
	if (pipe->is_3d) {
		...
		pipe->srcp0_ystride = msm_fb_line_length(0,
		 ...
		 /* 2D */
		pipe->src_height = mfd->ovr_dst_height;	/* fbi->var.yres; */
		pipe->src_width = mfd->ovr_dst_width;	/* fbi->var.xres; */
		pipe->src_h = mfd->ovr_src_height;	/* fbi->var.yres; */
		pipe->src_w = mfd->ovr_src_width;	/* fbi->var.xres; */
		pipe->dst_h = mfd->ovr_dst_height;	/* fbi->var.yres; */
		pipe->dst_w = mfd->ovr_dst_width;	/* fbi->var.xres; */
...
static void mdp4_overlay_update_dsi_cmd(struct msm_fb_data_type *mfd)
{
	...
	if (mfd->panel_info.read_pointer)
		MDP_OUTP(MDP_BASE + 0x021c, mfd->panel_info.read_pointer); /* read pointer */
	else
		MDP_OUTP(MDP_BASE + 0x021c, 100); /* default read pointer */
...
int mdp4_dsi_cmd_off(struct platform_device *pdev)
{
	...
	if (need_wait) {
		while (vctrl->clk_enabled){
			msleep(20);
			cnt++;
			if (0 == (cnt & 0x7f)) { /* about 2.5 seconds */
...
void mdp4_dsi_cmd_overlay(struct msm_fb_data_type *mfd)
{
	...
	/* HTC: if pan_display happened without vsync driven control,
	 *      turn on related clk and irq directly. And reset
	 *      expire_tick here to make sure rdptr_isr will turn off
	 *      clk and irq in clk_ctrl_work() later.
	 */
	if (vctrl->clk_enabled == 0) {
		clk_set_on = 1;
		vctrl->clk_enabled = 1;
