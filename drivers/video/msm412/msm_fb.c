...
/* HTC: Implement pan display perflock */
struct display_perflock {
	unsigned int expire_tick;

	struct pm_qos_request qos_req;
	struct perf_lock lock_0; /* for basic pan display */
	struct perf_lock lock_more; /* for pan display + others(eg.rotator) */
...
static int sre_ctl_switch(int level)
{
   ...
   sre_status = level;
   //PR_DISP_INFO("%s: change sre status\n", __func__);
...
/*
static ssize_t camera_backlight_value_store(struct device *dev,
			  ...
			  size_t count)
{
	...
	if (mfd->index == 0) { /* Only apply this hint on fb0 */
...
static int msm_fb_probe(struct platform_device *pdev)
{
	...
	/* HTC: Fix something for Android CTS*/
...
void esd_wq_routine(struct work_struct *work) {
	uint32 err_status = mipi_dsi_cmd_bta_sw_trigger_status(); /* clean up ack_err_status */
...
void msm_fb_set_backlight(struct msm_fb_data_type *mfd, __u32 bkl_lvl)
{
	...
	if (mfd->panel_power_on && !bl_updated && !mfd->request_display_on) {
		...
		unset_bl_level = 0;
		bl_updated = 1;
		bl_level_old = 0;	/* Force update backlight */
...
*/
	...
	default:
		if (mfd->panel_power_on) {
			...
			/* Delete dimming timer when display is going to off */
			if ((pdata) && (pdata->dimming_on))
...
int calc_fb_offset(struct msm_fb_data_type *mfd, struct fb_info *fbi, int bpp)
{
	...
	if (panel_info->mode2_yres != 0) {
		yres = mfd->ovr_src_height;	/* panel_info->mode2_yres; */
		remainder = (fbi->fix.line_length*yres) & (PAGE_SIZE - 1);
	} else {
		yres = mfd->ovr_src_height;	/* panel_info->yres; */
		remainder = (fbi->fix.line_length*yres) & (PAGE_SIZE - 1);
	}
...
static int msm_fb_register(struct msm_fb_data_type *mfd)
{
	...
	fix->line_length = msm_fb_line_length(mfd->index, mfd->ovr_src_width/*panel_info->xres*/,
					      bpp);

	/* Make sure all buffers can be addressed on a page boundary by an x
	 * and y offset */

	remainder = (fix->line_length * mfd->ovr_src_height/*panel_info->yres*/) & (PAGE_SIZE - 1);
	...
	if (!bf_supported || mfd->index == 0)
		fix->smem_len = MAX((msm_fb_line_length(mfd->index,
							mfd->ovr_src_width/*panel_info->xres*/,
							bpp) *
				     mfd->ovr_src_height/*panel_info->yres*/ + PAGE_SIZE -
	...
	mfd->var_xres = mfd->ovr_src_width;	/* panel_info->xres; */
	mfd->var_yres = mfd->ovr_src_height;	/* panel_info->yres; */
	mfd->var_frame_rate = panel_info->frame_rate;

	var->pixclock = mfd->panel_info.clk_rate;
	mfd->var_pixclock = var->pixclock;

	var->xres = mfd->ovr_src_width;	/* panel_info->xres; */
	var->yres = mfd->ovr_src_height;	/* panel_info->yres; */
	var->xres_virtual = ALIGN( mfd->ovr_src_width, 32);	/* panel_info->xres; */
	var->yres_virtual = mfd->ovr_src_height/*panel_info->yres*/ * mfd->fb_page +
	...
	mfd->panel_info.lcdc.underflow_clr = 0x0; /* black */
#endif
			      ...
			      struct fb_info *info)
{
	...
	mutex_lock(&msm_fb_blank_mutex);
	if (bf_supported && info->node == 2) {
		static unsigned int panfb2cnt = 0; /* ratelimit */
		if ((panfb2cnt & 0x3f)==0) { /* 63 frame, about 1.5 ~ 2 seconds */
	...
	/* HTC: If request mdp/rotator run in performance mode,
	   add pm_qos_req and perf_lock to prevent system idle */
	if (mfd->perfhint) {
		/* reset expire_tick should be outside of update_perflock() */
...
static int msm_fb_set_par(struct fb_info *info)
{
	...
	mfd->fbi->fix.line_length = msm_fb_line_length(mfd->index, mfd->ovr_src_width/*var->xres*/,
						       var->bits_per_pixel/8);
			...
			unsigned long arg)
{
	...
	case MSMFB_SET_DISP_PROJECTOR_INFO:
		/*
		 * getting hsml client size and device size for USB
		 * client size is used for USB dynamic buffer size
		 * deivce size is used for setting the resolution limit of hsm client
		 */
