...
ssize_t wimaxdbg_write(struct file *file, const char *buffer, unsigned long count, void *data)
{
    ...
    else if (dbg_para == 26) { // 26: Disable wakelock log
        printk(KERN_INFO "%s: mmc_wimax_set_sdio_wakelock_log:0\n", __func__);
        mmc_wimax_set_sdio_wakelock_log(0);
    }
    else if (dbg_para == 27) { // 27: Enable wakelock log
        printk(KERN_INFO "%s: mmc_wimax_set_irq_log_status:1\n", __func__);
        mmc_wimax_set_sdio_wakelock_log(1);
    }
    else if (dbg_para == 28) { // 28: Disable LSP power log
        printk(KERN_INFO "%s: mmc_wimax_set_sdio_lsp_log:0\n", __func__);
        mmc_wimax_set_sdio_lsp_log(0);
    }
    else if (dbg_para == 29) { // 29: Enable LSP power log
        printk(KERN_INFO "%s: mmc_wimax_set_sdio_lsp_log:1\n", __func__);
        mmc_wimax_set_sdio_lsp_log(1);
    }
    else if (dbg_para == 30) { // 30: Disable lite dump raw data for wakeup reason
        printk(KERN_INFO "%s: mmc_wimax_set_sdio_wakeup_lite_dump:0\n", __func__);
        mmc_wimax_set_sdio_wakeup_lite_dump(0);
    }
    else if (dbg_para == 31) { // 31: Enable lite dump raw data for wakeup reason
        printk(KERN_INFO "%s: mmc_wimax_set_sdio_wakeup_lite_dump:1\n", __func__);
        mmc_wimax_set_sdio_wakeup_lite_dump(1);
    }
	else if (dbg_para == 32) { // 32: Disable mmc_wimax_trigger_RD_FIFO_LEVEL_ERROR
        printk(KERN_INFO "%s: mmc_wimax_trigger_RD_FIFO_LEVEL_ERROR:0\n", __func__);
        mmc_wimax_trigger_RD_FIFO_LEVEL_ERROR(0);
    }
	else if (dbg_para == 33) { // 33: Enable mmc_wimax_trigger_RD_FIFO_LEVEL_ERROR
        printk(KERN_INFO "%s: mmc_wimax_trigger_RD_FIFO_LEVEL_ERROR:1\n", __func__);
        mmc_wimax_trigger_RD_FIFO_LEVEL_ERROR(1);
    }
	else if (dbg_para == 34) { // 34: Disable mmc_wimax_set_wimax_FW_freeze_WK_RX
        printk(KERN_INFO "%s: mmc_wimax_set_wimax_FW_freeze_WK_RX:0\n", __func__);
        mmc_wimax_set_wimax_FW_freeze_WK_RX(0);
    }
	else if (dbg_para == 35) { // 35: Enable mmc_wimax_set_wimax_FW_freeze_WK_RX
        printk(KERN_INFO "%s: mmc_wimax_set_wimax_FW_freeze_WK_RX:1\n", __func__);
        mmc_wimax_set_wimax_FW_freeze_WK_RX(1);
    }
	else if (dbg_para == 36) { // 36: Disable mmc_wimax_set_wimax_FW_freeze_WK_TX
        printk(KERN_INFO "%s: mmc_wimax_set_wimax_FW_freeze_WK_TX:0\n", __func__);
        mmc_wimax_set_wimax_FW_freeze_WK_TX(0);
    }
	else if (dbg_para == 37) { // 37: Enable mmc_wimax_set_wimax_FW_freeze_WK_TX
        printk(KERN_INFO "%s: mmc_wimax_set_wimax_FW_freeze_WK_TX:1\n", __func__);
        mmc_wimax_set_wimax_FW_freeze_WK_TX(1);
    }
