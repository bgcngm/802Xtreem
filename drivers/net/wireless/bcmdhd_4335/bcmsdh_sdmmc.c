...
#endif /* (CUSTOMER_HW4 || CUSTOMER_HW2) && USE_DYNAMIC_F2_BLKSIZE */
		break;
	}
...
sdioh_request_byte(sdioh_info_t *sd, uint rw, uint func, uint regaddr, uint8 *byte)
{
	...
	if (err_ret) {
		/*	XXX: Read/Write to SBSDIO_FUNC1_SLEEPCSR could return -110(timeout) error,
		*		in case the host tries to wake the device up.
		*/
                     ...
                     uint addr, void *pkt)
{
	...
	if (0 != dma_len) {
		...
		memset(&mmc_dat, 0, sizeof(struct mmc_data));
                  ...
                  //HTC_CSP_START
                  //20130104 Luke : make CONFIG_DEBUG_SG=y happy
