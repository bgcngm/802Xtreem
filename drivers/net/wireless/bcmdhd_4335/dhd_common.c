...
/* HTC_WIFI_START */
static int print_pkt_filter_list(dhd_pub_t *);
/* HTC_WIFI_END */
...
static int
print_pkt_filter_list(dhd_pub_t * dhd)
{
    ...
    if (!dhd) {
        printf("print_get pkt_filter_list: dhd is null");
        return -1;
    }
    bcm_mkiovar("pkt_filter_list", (char *)&enable, 4, buf, sizeof(buf));
    if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_GET_VAR, buf, sizeof(buf),
                    FALSE, 0)) < 0) {
        printf("print_pkt_filter_list: ret = %d", ret);
        return -1;
    }
    else {
