 ...
 * Short to 0.1 second (100000us) for save time.
 */
#if (PMU_MAX_TRANSITION_DLY <= 100000)
#undef PMU_MAX_TRANSITION_DLY
#define PMU_MAX_TRANSITION_DLY 100000
#endif
...
//static struct regulator *wifi_vddcx;
...
/*static int msm_wifi_config_vddcx(int high)
{
	int ret;

	if(high)
	{
		ret = regulator_set_voltage(wifi_vddcx, 1150000, 1150000);
	}
	else
		ret = regulator_set_voltage(wifi_vddcx, 950000, 1150000);
	if (ret) {
		DHD_TRACE(("unable to set the voltage for regulator WIFI_VDDCX\n"));
		return ret;
	}
...
#endif
		//msm_wifi_config_vddcx(0);
		//regulator_disable(wifi_vddcx);
	} else {
		//msm_wifi_config_vddcx(1);
		//regulator_enable(wifi_vddcx);
		/* Exit Sleep */
		/* Make sure we have SD bus access */
		if (bus->clkstate == CLK_NONE) {
		...
		if (ip_tcp_ttllen ==  ip_hdr_len + tcp_hdr_len) {
			...
			*/
			...
			if (tcp_ack_info->p_tcpackinqueue) {
				...
				} else {	/* XXX Should get rid of these packets? */
#ifdef TCPACK_TEST
					void *prevpkt = tcp_ack_info->p_tcpackinqueue;
#endif
...
/* HTC_CSP_END */
	...
	if (bus->dhd->txcnt_timeout >= max_cntl_timeout/*MAX_CNTL_TIMEOUT*/) //2012-12-06 brcm 
		return -ETIMEDOUT;
...
dhd_bus_rxctl(struct dhd_bus *bus, uchar *msg, uint msglen)
{
	...
	if (bus->dhd->rxcnt_timeout >= max_cntl_timeout/*MAX_CNTL_TIMEOUT*/) //2012-12-06 brcm
		return -ETIMEDOUT;
...
dhd_bus_init(dhd_pub_t *dhdp, bool enforce_mutex)
{
	...
	//wifi_vddcx = regulator_get(NULL, "WIFI_VDDCX");
...
dhdsdio_rxglom(dhd_bus_t *bus, uint8 rxseq)
{
	...
	if (bus->glom) {
		...
		if (errcode < 0) {
			...
			rxfail_count++;
			if (rxfail_count > rxglom_fail_count/*RXGLOM_FAIL_COUNT*/) {
				dhd_info_send_hang_message(bus->dhd);
				rxfail_count = 0;
			}
			return 0;
...
/* PR94636 : SD3.0 SDR50/SDR104/DDR50 mode CMD53 byte mode times out.
 * For sdiod DMA Tx channel when cd = ld => sdiod h/w will send 0's if host tries to do a F2 read.
 * While host does a F2 read & dongle sw tries to post a new DMA Tx descriptor
 * sdiod device h/w is getting hanged.
 * This host WAR ensures from host we are avoiding a F2 read of 0's bytes.
*/
	...
	uint16 func, uint bustype, void *regsva, osl_t * osh, void *sdh)
{
	...
	//bus->glom_mode = bcmsdh_set_mode(bus->sdh, 1);
