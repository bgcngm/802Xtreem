...
/* HTC_CSP_END */
	...
	// brcm 2012-10-31
	printf("%s: dhd->pub.hang_was_sent ==> %d\n",__func__ ,dhd->pub.hang_was_sent);
	dhd->pub.hang_was_sent = 0;
	// end
#if !defined(WL_CFG80211)
	/*
...
exit:
	// brcm 2012-10-31 : dhd_open failed, then do wifi_off/dhd_stop to clear/complete wifi off procedure.
	if (ret) {
		wl_android_wifi_off(net);
		dhd_stop(net);
	}
