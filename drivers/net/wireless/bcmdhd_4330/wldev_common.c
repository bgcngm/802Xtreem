...
wldev_set_apsta(struct net_device *dev, bool enable)
{
	...
	if (enable){
		...
		ba_size = 8;
		if ((res = wldev_iovar_setint(dev, "ampdu_ba_wsize", ba_size))) {
			printf("%s fail to set ampdu_ba_wsize res[%d]\n", __FUNCTION__,res);
		}

		srl = lrl = 7;
		wldev_ioctl(dev, WLC_SET_SRL, &srl, sizeof(srl), 1);
		wldev_ioctl(dev, WLC_SET_LRL, &lrl, sizeof(lrl), 1);
		//Hugh 2012-09-10 CT2 set Concr_mode ++++
