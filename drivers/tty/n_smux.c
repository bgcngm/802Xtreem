...
/* SSD_RIL: the delay for the first wakeup attempt */
#define SMUX_WAKEUP_DELAY_START (1 << 8)

/* SSD_RIL: the tolerance for the usleep_range */
#define SMUX_WAKEUP_DELAY_TOLERANCE (1 << 8)

/* SSD_RIL: minimum delay for scheduling delayed work */
#define SMUX_WAKEUP_DELAY_TRANSITION (1 << 15)

/* maximum sleep time between wakeup attempts */
#define SMUX_WAKEUP_DELAY_MAX (1 << 14)
...
#define KERNEL_FLAG_RIL_DBG_SMUX_IPC KERNEL_FLAG_RIL_DBG_DATA_LPM
#define KERNEL_FLAG_RIL_DBG_SMUX_RAW_DATA KERNEL_FLAG_RIL_DBG_CTL

#define AP2QSC_STATUS_GPIO 44
...
static int smux_debug_mask=0xFF;
...
static int power_collapse_disable;

void trigger_qsc_fatal_err(const char *reason);
#ifdef CONFIG_SMUX_WORKAROUND_QSC_WAKEUP_ACK
...
static void smux_log_pkt(struct smux_pkt_t *pkt, int is_recv)
{
	...
	i += snprintf(logbuf + i, SMUX_PKT_LOG_SIZE - i,
			"smux_ch%d: %c%c:%c%c %s%s flags %x len %d:%d \n"
			"smux_ch%d: %s",
			pkt->hdr.lcid,
...
static int smux_handle_rx_power_cmd(struct smux_pkt_t *pkt)
{
	...
	if (pkt->hdr.flags & SMUX_CMD_PWR_CTL_ACK) {
		...
		if (smux.power_state == SMUX_PWR_ON) {
			...
			/* Trigger QSC fatal and only for debug*/
			pr_err("%s: Trigger QSC fatal for debug purpose.\n", __func__);
			trigger_qsc_fatal_err("sleep request invalid in state");
		}
	}
...
static void smux_handle_wakeup_req(void)
{
	...
	if (smux.power_state == SMUX_PWR_OFF
		...
		smux.power_state = SMUX_PWR_ON;
		/* +SSD_RIL: to avoid wakeup worker starting with a huge delay
						if an off/on cycle has been completed before the next delayed work start. */
		smux.pwr_wakeup_delay_us = SMUX_WAKEUP_DELAY_START;
		/* -SSD_RIL: to avoid wakeup worker starting with a huge delay
						if an off/on cycle has been completed before the next delayed work start. */
		...
		int len, int *used, int flag)
{
	...
	for (i = *used; i < len && smux.rx_state == SMUX_RX_HDR; i++) {
		...
		if (smux.recv_len == sizeof(struct smux_hdr_t)) {
			...
			/* SSD_RIL: Trigger QSC fatal if packet too big. */
			if (unlikely(hdr->payload_len + hdr->pad_len + sizeof(struct smux_hdr_t) > SMUX_MAX_PKT_SIZE))
			{
				trigger_qsc_fatal_err("SMUX_PKT_Overflow");
				smux_enter_reset();
				return;
			}
			else
...
static void smux_wakeup_worker(struct work_struct *work)
{
	...
	if (smux.power_state == SMUX_PWR_ON) {
		...
		if (smux.pwr_wakeup_delay_us > SMUX_WAKEUP_DELAY_MAX)
			smux.pwr_wakeup_delay_us = SMUX_WAKEUP_DELAY_MAX;

		/* SSD_RIL: if remote is not alive, don't ping so frequently. */
		if(unlikely(!smux.remote_is_alive))
			wakeup_delay = 1000000;
		else
		{
			if(wakeup_delay == SMUX_WAKEUP_DELAY_MAX)
				wakeup_counter++;
			else
				wakeup_counter = 0;
			...
			{
				SMUX_ERR("%s: No wakeup ack received for more than 100 wakeup req.\n", __func__);
				wakeup_counter = 0;
				trigger_qsc_fatal_err("No_Wakeup_Ack");
			}
		}
		...
		if (wakeup_delay < SMUX_WAKEUP_DELAY_TRANSITION) {
			SMUX_DBG("smux: %s: sleeping for %u us\n", __func__,
					wakeup_delay);
			usleep_range(wakeup_delay, wakeup_delay+SMUX_WAKEUP_DELAY_TOLERANCE);
			queue_work(smux_tx_wq, &smux_wakeup_work);
		} else {
#ifdef CONFIG_SMUX_WORKAROUND_QSC_WAKEUP_ACK
			if( get_qsc_wakeup_ack_gpio() && smux.remote_is_alive)
			{
				const unsigned char fake_ack = SMUX_WAKEUP_ACK;

				SMUX_ERR( "%s: Hit workaround. Fake wakeup ack packet.", __func__);
				smux_rx_state_machine(&fake_ack, 1, 0);
			}
#endif
...
static void smux_rx_worker(struct work_struct *work)
{
	...
	DbgBuffer_printRaw((void*)data, len, "SMUX_RX");
...
int msm_smux_write(uint8_t lcid, void *pkt_priv, const void *data, int len)
{
	...
	if (len > SMUX_MAX_PKT_SIZE - sizeof(struct smux_hdr_t)) {
		SMUX_ERR("%s: ch %d payload %d too large\n",
				__func__, lcid, len);
		ret = -E2BIG;
		goto out;
	}
	...
	SMUX_DBG("smux: %s: ch %d pending %d", __func__, lcid, ch->tx_pending_data_cnt);
	...
	if (++ch->tx_pending_data_cnt == SMUX_TX_WM_HIGH) {
		ch->notify_lwm = 1;
		SMUX_ERR("%s: ch %d high watermark hit\n", __func__, lcid);
...
/*++SSD_RIL*/
    ...
    if (boot_mode == 1 || boot_mode ==  6 || boot_mode == 8)
		power_collapse_disable = 1;
    pr_info("%s boot_mode = %d, power_collapse_disable = %d\n", __func__, boot_mode, power_collapse_disable);
/*--SSD_RIL*/
	if (ret != 0) {
	...
	subsys_notif_register_notifier("qsc_modem", &ssr_notifier);
	...
	if (!(get_kernel_flag() & KERNEL_FLAG_RIL_DBG_SMUX_IPC))
		smux_debug_mask &= ~(MSM_SMUX_DEBUG | MSM_SMUX_POWER_INFO);
	if (!(get_kernel_flag() & KERNEL_FLAG_RIL_DBG_SMUX_RAW_DATA))
		smux_debug_mask &= ~MSM_SMUX_PKT;
