...
#define pr_handshaking_debug(x...) do {                             \
...
#ifdef CONFIG_IMC_UART2DM_HANDSHAKE
	unsigned char pda_int_bb;		/* low: BB can sleep */
	unsigned char bb_int_pda;		/* low: PDA can sleep */
...
static int msm_hs_init_clk(struct uart_port *uport)
{
	...
	if (ret) {
		printk(KERN_WARNING "[GSM_RADIO] Error setting clock rate on UART\n");
		return ret;
	}

	ret = clk_prepare_enable(msm_uport->clk);
	if (ret) {
		printk(KERN_WARNING "[GSM_RADIO] Error could not turn on UART clk\n");
		return ret;
	}
	if (msm_uport->pclk) {
		...
		if (ret) {
			...
			dev_err(uport->dev,
				"[GSM_RADIO] Error could not turn on UART pclk\n");
			return ret;
		}
	}
 ...
 */
unsigned int imc_msm_hs_tx_empty(struct uart_port *uport)
{
	unsigned int data;
	unsigned int ret = 0;
	//struct msm_hs_port *msm_uport = UARTDM_TO_MSM(uport);
...
static void msm_hs_submit_tx_locked(struct uart_port *uport)
{
	...
	if (uart2_debug_mask && !modem_fatal) {
		printk("[GSM_RADIO] Tx: ");
		for (idx=0 ; idx<tx_count; idx++)
			printk("%02x ", tx_buf->buf[tx_buf->tail + idx]);
		printk("\n");
	}

	// Debugging purpose
	if (tx_buf->buf[tx_buf->tail] == 0x61 && tx_buf->buf[tx_buf->tail + 1] == 0x54) {
		printk("[GSM_RADIO] receive aT command!\n");
		printk("[GSM_RADIO] XMM status: %d\n", gpio_get_value(GPIO_GSM_XMM_AP_STATUS));
		printk("[GSM_RADIO] AP status: %d\n", gpio_get_value(GPIO_GSM_AP_XMM_STATUS));
		printk("[GSM_RADIO] XMM wakeup: %d\n", gpio_get_value(GPIO_GSM_XMM_AP_WAKE));
		printk("[GSM_RADIO] AP wakeup: %d\n", gpio_get_value(GPIO_GSM_AP_XMM_WAKE));
	}
	...
	*tx->command_ptr_ptr = CMD_PTR_LP | DMOV_CMD_ADDR(tx->mapped_cmd_ptr);
	...
	if ((tx->xfer.list.next != LIST_POISON1) &&	(tx->xfer.list.next != NULL))
		printk(KERN_WARNING "[GSM_RADIO] Error: tx already started in dmov\n");
...
static void msm_hs_start_rx_locked(struct uart_port *uport)
{
	...
	if (buffer_pending && hs_serial_debug_mask)
		printk(KERN_WARNING "[GSM_RADIO] Error: rx started in buffer state = %x",
		       buffer_pending);
	...
	if ((msm_uport->rx.xfer.list.next != LIST_POISON1) && (msm_uport->rx.xfer.list.next != NULL))
		printk(KERN_WARNING "[GSM_RADIO] Error: rx already started in dmov\n");
...
static void flip_insert_work(struct work_struct *work)
{
	...
	if (msm_uport->rx.buffer_pending == NONE_PENDING) {
		if (hs_serial_debug_mask)
			printk(KERN_WARNING "[GSM_RADIO] Error: No buffer pending in %s",
			       __func__);
		return;
	}
...
static void msm_serial_hs_rx_tlet(unsigned long tlet_ptr)
{
	...
	if (uart2_debug_mask && !modem_fatal) {
		...
		for (idx=0 ; idx<rx_count; idx++)
			printk("%02x ", msm_uport->rx.buffer[idx]);
		printk("\n");
	}
	...
	if (is_gsbi_uart(msm_uport)) {
		...
		if (msm_uport->pclk) {
			...
			if (radio_state) {
				...
				pr_handshaking_debug("%s PDA_STATUS -\n",__func__);
			}
			else{
				pr_handshaking_debug("%s NOT PDA_STATUS - due to radio_off\n",__func__);
			}
#endif
...
/*#ifdef CONFIG_IMC_UART2DM_HANDSHAKE
	...
	/*if (msm_uport->clk_state != MSM_HS_CLK_REQUEST_OFF ||
		...
		!(msm_uport->imr_reg & UARTDM_ISR_RXLEV_BMSK)) {
		pr_debug(KERN_INFO "%s did not clock off\n", __FUNCTION__);
		return -1;
	}*/

	if (msm_uport->clk_state != MSM_HS_CLK_REQUEST_OFF){
		pr_handshaking_debug("%s ret=-1(clk_state != REQUEST_OFF) -\n", __func__);
		return -1;
	}
	if (!uart_circ_empty(tx_buf)){
		pr_handshaking_debug("%s ret=-1(!uart_circ_empty) -\n", __func__);
		return -1;
	}
	if (msm_uport->tx.dma_in_flight){
		pr_handshaking_debug("%s ret=-1(tx.dma_in_flight) -\n", __func__);
		return -1;
	}
	if (msm_uport->imr_reg & UARTDM_ISR_TXLEV_BMSK){
		pr_handshaking_debug("%s ret=-1(ISR_TXLEV) -\n", __func__);
		return -1;
	}
	if (!(msm_uport->imr_reg & UARTDM_ISR_RXLEV_BMSK)) {
		pr_handshaking_debug("%s ret=-1(!ISR_RXLEV) -\n", __func__);
		return -1;
	}
	...
	if (!(sr_status & UARTDM_SR_TXEMT_BMSK)) {
		pr_handshaking_debug(KERN_INFO "%s (!SR_TXEMT)\n", __FUNCTION__);
		return 0;  /* retry */
		}

#ifdef CONFIG_IMC_UART2DM_HANDSHAKE
	/* PDA has nothing send to BB, so set PDA_INT_BB to low */
	if (msm_uport->pda_int_bb == 1) { /*PDA_INT_BB high: BB cannot sleep */
		gpio_set_value(GPIO_GSM_AP_XMM_WAKE, 0);
		pr_handshaking_debug("%s PDA_INT_BB -\n",__func__);
		msm_uport->pda_int_bb = 0; /* PDA_INT_BB high : BB can sleep */
	}

	/* now we can start to check rx part	*/
	/* check bb_wakeup pin level */
	if (gpio_get_value(GPIO_GSM_XMM_AP_WAKE)
			|| (msm_uport->bb_int_pda == 1)) { /* BB_INT_PDA high : PDA cannot sleep */
		//msm_uport->clk_state = MSM_HS_CLK_ON;
		pr_handshaking_debug("%s ret=-1(bb_int_pda=1 or BB_INT_PDA +)\n", __func__);
		return -1;	/* host_wake is still low, no need to retry */
	} else {
		if (sr_status & UARTDM_SR_RXRDY_BMSK){
			pr_handshaking_debug("%s ret=0(RXRDY) -\n", __func__);
			return 0;  /* some data in rx fifo, so retry */
		}
	}
#endif
	pr_handshaking_debug("%s msm_uport->clk_req_off_state=%d\n", __func__, msm_uport->clk_req_off_state);
	...
	case CLK_REQ_OFF_START:
		 ...
		 */
		mb();
		pr_handshaking_debug("%s ret=0(CLK_REQ_OFF_START) -\n", __func__);
		return 0;  /* RXSTALE flush not complete - retry */
	case CLK_REQ_OFF_RXSTALE_ISSUED:
	case CLK_REQ_OFF_FLUSH_ISSUED:
		pr_handshaking_debug("%s ret=0(CLK_REQ_OFF_RXSTALE_ISSUED or CLK_REQ_OFF_FLUSH_ISSUED) -\n", __func__);
		return 0;  /* RXSTALE flush not complete - retry */
	...
	if (msm_uport->rx.flush != FLUSH_SHUTDOWN) {
		...
		pr_handshaking_debug("%s ret=0(!FLUSH_SHUTDOWN) -\n", __func__);
	...
	if (use_low_power_wakeup(msm_uport) && !modem_fatal) {
		msm_uport->wakeup.ignore = 1;
#ifdef CONFIG_IMC_UART2DM_HANDSHAKE
		pr_handshaking_debug("%s NOT enable_irq wakeup.irq(%d)\n",__func__, msm_uport->wakeup.irq);
#else
		enable_irq(msm_uport->wakeup.irq);
#endif
	}
	wake_unlock(&msm_uport->dma_wake_lock);
	pr_handshaking_debug("%s Clock OFF\n",__func__);
	return 1;
}
...
void imc_msm_hs_request_clock_off_locked(struct uart_port *uport)
{
	struct msm_hs_port *msm_uport = UARTDM_TO_MSM(uport);
...
void imc_msm_hs_request_clock_off(struct uart_port *uport)
{
	unsigned long flags;

	spin_lock_irqsave(&uport->lock, flags);
	imc_msm_hs_request_clock_off_locked(uport);
	spin_unlock_irqrestore(&uport->lock, flags);

}

void imc_msm_hs_request_clock_on_locked(struct uart_port *uport)
{
	struct msm_hs_port *msm_uport = UARTDM_TO_MSM(uport);
	unsigned int data;
	int ret = 0;
	...
	case MSM_HS_CLK_OFF:
		pr_handshaking_debug("%s clk_state=MSM_HS_CLK_OFF +\n", __func__);
		wake_lock(&msm_uport->dma_wake_lock);
		clk_prepare_enable(msm_uport->clk);
		if (msm_uport->pclk)
			ret = clk_prepare_enable(msm_uport->pclk);
#ifdef CONFIG_IMC_UART2DM_HANDSHAKE
		pr_handshaking_debug("%s NOT disable_irq wakeup.irq(%d)\n",__func__, msm_uport->wakeup.irq);
#else
		disable_irq_nosync(msm_uport->wakeup.irq);
#endif
		if (unlikely(ret)) {
			dev_err(uport->dev, "[GSM_RADIO] Clock ON Failure"
				"Stalling HSUART\n");
			break;
		}
		/* else fall-through */
	case MSM_HS_CLK_REQUEST_OFF:
		pr_handshaking_debug("%s clk_state=MSM_HS_CLK_REQUEST_OFF +\n", __func__);
/*
#ifdef CONFIG_IMC_UART2DM_HANDSHAKE
		msm_uport->request_clk_off_delay = 100; //10ms*100 = 1000ms
#endif
*/
		...
		if (msm_uport->rx.flush == FLUSH_STOP)
			msm_uport->rx.flush = FLUSH_IGNORE;
		msm_uport->clk_state = MSM_HS_CLK_ON;
		pr_handshaking_debug("%s clk_state=MSM_HS_CLK_ON -\n", __func__);
#ifdef CONFIG_IMC_UART2DM_HANDSHAKE
		if (radio_state) {
...
void imc_msm_hs_request_clock_on(struct uart_port *uport) {
	unsigned long flags;

#ifdef CONFIG_IMC_UART2DM_HANDSHAKE
		struct msm_hs_port *msm_uport = UARTDM_TO_MSM(uport);
		msm_uport->pda_int_bb = 1; /*PDA_INT_BB low: BB cannot sleep */
#endif
	spin_lock_irqsave(&uport->lock, flags);
	imc_msm_hs_request_clock_on_locked(uport);
	spin_unlock_irqrestore(&uport->lock, flags);
}
//EXPORT_SYMBOL(imc_msm_hs_request_clock_on);
...
static irqreturn_t msm_hs_wakeup_isr(int irq, void *dev)
{
	...
	if (!gpio_get_value(GPIO_GSM_XMM_AP_WAKE)) { /*BB_INT_PDA high: PDA cannot sleep */
		pr_handshaking_debug(" - \n");
		msm_uport->request_clk_off_delay = 30; //100ms*30 = 3s
		msm_uport->bb_int_pda = 0; /*BB_INT_PDA low: PDA can sleep */
		msm_uport->clk_state = MSM_HS_CLK_REQUEST_OFF;
		msm_uport->clk_req_off_state = CLK_REQ_OFF_START;
...
static int msm_hs_startup(struct uart_port *uport)
{
	...
	printk(KERN_INFO "[GSM_RADIO] ++ S UP ++\n");
	...
	if (use_low_power_wakeup(msm_uport) && !modem_fatal) {
		...
		if (unlikely(ret)) {
			printk(KERN_WARNING "[GSM_RADIO] Err setting wakeup irq!\n");
			goto deinit_uart_clk;
		}
	}

	ret = request_irq(uport->irq, msm_hs_isr, IRQF_TRIGGER_HIGH,
			  "msm_hs_uart_imc", msm_uport);
	if (unlikely(ret))
	{
		printk(KERN_WARNING "[GSM_RADIO] request irq failed!\n");
		goto free_wake_irq;
	}

	if (use_low_power_wakeup(msm_uport) && !modem_fatal) {
#ifdef CONFIG_IMC_UART2DM_HANDSHAKE
		wakeup_irq = msm_uport->wakeup.irq;
		msm_uport->bb_int_pda = 0; /*BB_INT_PDA low: PDA can sleep */
#endif
		ret = request_irq(msm_uport->wakeup.irq, msm_hs_wakeup_isr,
				  IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING,
				  "msm_hs_wakeup_imc", msm_uport);
		wakeup_irq_ret = ret;
		if (unlikely(ret))
		{
			printk(KERN_WARNING "[GSM_RADIO] request wakeup irq failed!\n");
			goto free_uart_irq;
		}
...
/*
	...
	if (ret)
		dev_err(uport->dev, "[GSM_RADIO]set active error:%d\n", ret);
...
static int msm_hs_probe(struct platform_device *pdev)
{
	...
	if (pdev->id < 0 || pdev->id >= UARTDM_NR) {
		printk(KERN_WARNING "Invalid plaform device ID = %d\n", pdev->id);
		return -EINVAL;
	}

	printk(KERN_INFO "[GSM_RADIO] msm_hs_probe\n");
	...
	*/
	...
	uport->line = pdev->id;
	printk(KERN_INFO "[GSM_RADIO] msm_hs_probe complete\n");
...
static int __init msm_serial_hs_init(void)
{
	...
	if (unlikely(ret)) {
		printk(KERN_WARNING "[GSM_RADIO] %s failed to load\n", __FUNCTION__);
		return ret;
	}
	...
	printk(KERN_INFO "[GSM_RADIO] msm_serial_hs module loaded\n");
	return ret;
}
...
static void msm_hs_shutdown(struct uart_port *uport)
{
	...
	printk(KERN_INFO "[GSM_RADIO] ++ S DN ++\n");
	...
	if (msm_uport->clk_state != MSM_HS_CLK_OFF) {
		clk_disable_unprepare(msm_uport->clk);  /* to balance clk_state */
		if (msm_uport->pclk)
			clk_disable_unprepare(msm_uport->pclk);
		wake_unlock(&msm_uport->dma_wake_lock);
	}
	...
	/*  move out irq operation from spin_lock_irq range to prevent deadlock */
	/* disable irq wakeup when shutdown **/
	if (use_low_power_wakeup(msm_uport) && !wakeup_irq_ret)
		irq_set_irq_wake(msm_uport->wakeup.irq, 0);
