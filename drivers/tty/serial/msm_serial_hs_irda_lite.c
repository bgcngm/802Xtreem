...
#define D(x...) pr_info("[IrDA] " x)
#define E(x...) pr_err("[IrDA err] " x)
...
static void msm_hsl_config_port_irda(struct uart_port *port, int flags)
{
	...
	if (flags & UART_CONFIG_TYPE) {
		...
		if (msm_hsl_request_port_irda(port)) {
			D("%s () request_port_irda return:port->line %d, ir\n", __func__, port->line);
			return;
		}
	}
	if (msm_serial_hsl_has_gsbi(port)) {
		...
		D("%s: irda line[%d] GSBI_CONTROL_ADDR-%x\n", __func__,
