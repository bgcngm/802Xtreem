...
static void sdio_select_driver_type(struct mmc_card *card)
{
	...
	card->host->caps |= MMC_CAP_DRIVER_TYPE_A|MMC_CAP_DRIVER_TYPE_C|MMC_CAP_DRIVER_TYPE_D; // force driving type
#else
	if (!(card->host->caps &
...
int mmc_attach_sdio(struct mmc_host *host)
{
	...
	/* Support UHS mode only when we define SDR104 */
...
int sdio_reset_comm(struct mmc_card *card)
{
	...
	/* HTC_CSP_START -- 20121126 */
	if(isuhs) {
		printk("%s(): Ultra High speed mode\n", __func__);
		return mmc_power_restore_host(card->host);
	} else {
	/* HTC_CSP_END -- 20121126 */
