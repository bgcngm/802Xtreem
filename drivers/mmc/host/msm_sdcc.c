...
#include <mach/msm_rtb_disable.h> //HTC_WIFI: to improve throughput
		 ...
		 unsigned int status)
{
	if (status & MCI_DATACRCFAIL) {
		...
		/* In case of DATA CRC error, execute tuning again */
		...
		/* In case of DATA timeout error, execute tuning again */
		if (host->tuning_needed && !host->tuning_in_progress)
			host->tuning_done = false;
...
static void msmsdcc_do_cmdirq(struct msmsdcc_host *host, uint32_t status)
{
	...
	if (status & (MCI_CMDTIMEOUT | MCI_AUTOCMD19TIMEOUT)) {
		/*
		 * Print SD card Command timeout error except the MMC_SLEEP_AWAK (CMD 5),
		 * SD_SEND_IF_COND (CMD8) and SD_IO_RW_DIRECT (CMD 52)
		 * because it's the normal situation
		 */
		if (is_sd_platform(host->plat)) {
		...
		pr_err("%s: CMD%d: Command CRC error\n",
			mmc_hostname(host->mmc), cmd->opcode);
		msmsdcc_dump_sdcc_state(host);
		/* In case of CMD CRC error, execute tuning again */
		if (host->tuning_needed)
			host->tuning_done = false;
		if(is_sd_platform(host->plat)) {
			host->mmc->caps &= ~MMC_CAP_UHS_SDR104;
...
msmsdcc_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	if (host->tuning_needed && !host->tuning_in_progress &&
		...
		if (host->mmc->ios.timing == MMC_TIMING_UHS_SDR104) {
			if (host->cont_tuning_cnt <= MAX_CONTINUOUS_TUNING_COUNT) //HTC_WIFI: to avoid print unnecessary message
...
#ifdef CONFIG_WIFI_MMC
	if (is_wifi_platform(host->plat)) {
		printk(KERN_INFO "[WIFI] [MMC] %s msmsdcc_enable\n", mmc_hostname(host->mmc));
	}
#endif
*/
...
#ifdef CONFIG_WIFI_MMC
	if (is_wifi_platform(host->plat)) {
		printk(KERN_INFO "[WIFI] [MMC] %s msmsdcc_disable\n", mmc_hostname(host->mmc));
	}
#endif
*/
...
/* Workaround for Hynix 0x81/0x83 emmc.
 * Device need to issue CMD6 when rebooting to avoid auto BKOPS,
 * or device will enter QDLOAD mode.
 */
...
msmsdcc_suspend(struct device *dev)
{
	...
	if (is_wimax_platform(host->plat) && mmc_wimax_get_status())
		...
		/*HTC_CSP_START*/
		{
			...
			/* Disable suspend function for wimax slot*/
			if (!is_wimax_platform(host->plat)) 
#endif
			{
...
msmsdcc_resume(struct device *dev)
{
	...
	if (is_wimax_platform(host->plat) && mmc_wimax_get_status()) {
		...
		/*HTC_CSP_START*/
#ifdef CONFIG_WIMAX
		/* Disable resume function for wimax */
		if (!is_wimax_platform(host->plat))
#endif
		{
		       ...
		       size_t count, loff_t *ppos)
{
	...
	if (prealloc_size > 10 * 1024 * 1024)
		prealloc_size = 10 * 1024 * 1024; /* max 10MB */
	return count;
}
