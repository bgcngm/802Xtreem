...
static int ipc_bridge_reset_resume(struct usb_interface *intf)
{
	pr_info("%s intf %p\n", __func__, intf);
	return ipc_bridge_resume(intf);
}
...
ipc_bridge_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
	...
	//HTC: we enable auto suspend in qcserial
...
static struct usb_driver ipc_bridge_driver = {
    ...
    .reset_resume = ipc_bridge_reset_resume,
