...
/*
 * Check if the diag_request comes from htc_diag
 * If so, it should not notify Diag channel that write complete because we did not
 * need to free Diag channel memory (We allocate local memory for htc_diag).
 */
...
#endif
		// Disable ACM if ##diag is triggered
