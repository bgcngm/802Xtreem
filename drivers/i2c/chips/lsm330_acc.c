...
static int lsm330_acc_get_data(struct lsm330_acc_data *acc, int *xyz)
{
	...
	/*if (DEBUG_FLAG == 2) {
		D("%s: test i2c write debug logs!\n", __func__);
		err = lsm330_acc_i2c_update(acc, LSM330_CTRL_REG4,
				LSM330_XYZ_BDU_MASK, 0x0);
		if (err < 0)
			dev_err(&acc->client->dev, "%s: Debug: err = %d\n", __func__, err);
	}*/
		...
		const struct i2c_device_id *id)
{
	...
	calibration_version = gyro_gsensor_kvalue[36];// check if new calibration tool
