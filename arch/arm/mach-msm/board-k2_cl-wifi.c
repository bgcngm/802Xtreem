/* linux/arch/arm/mach-msm/board-k2_cl-wifi.c
*/
...
#include "board-k2_cl-wifi.h"

int k2_cl_wifi_power(int on);
int k2_cl_wifi_reset(int on);
int k2_cl_wifi_set_carddetect(int on);
int k2_cl_wifi_get_mac_addr(unsigned char *buf);
...
static void *k2_cl_wifi_mem_prealloc(int section, unsigned long size)
{
...
int __init k2_cl_init_wifi_mem(void)
{
	int i;
...
static struct resource k2_cl_wifi_resources[] = {
	[0] = {
...
static struct wifi_platform_data k2_cl_wifi_control = {
	.set_power      = k2_cl_wifi_power,
	.set_reset      = k2_cl_wifi_reset,
	.set_carddetect = k2_cl_wifi_set_carddetect,
	.mem_prealloc   = k2_cl_wifi_mem_prealloc,
	.get_mac_addr	= k2_cl_wifi_get_mac_addr,
	.bus_scale_table    = &wlan_bus_scale_pdata,
};

static struct platform_device k2_cl_wifi_device = {
	.name           = "bcmdhd_wlan",
	.id             = 1,
	.num_resources  = ARRAY_SIZE(k2_cl_wifi_resources),
	.resource       = k2_cl_wifi_resources,
	.dev            = {
		.platform_data = &k2_cl_wifi_control,
	},
};

static unsigned k2_cl_wifi_update_nvs(char *str)
{
...
int k2_cl_wifi_get_mac_addr(unsigned char *buf)
{
	...
	printk(KERN_INFO"k2_cl_wifi_get_mac_addr = %02x %02x %02x %02x %02x %02x \n",
...
#else
	k2_cl_wifi_update_nvs("sd_oobonly=1\n");
#endif
	k2_cl_wifi_update_nvs("btc_params80=0\n");
	k2_cl_wifi_update_nvs("btc_params6=30\n");
	k2_cl_init_wifi_mem();
	ret = platform_device_register(&k2_cl_wifi_device);
	return ret;
}
