...
static int valente_wx_detect_panel(const char *name)
{
...
int valente_wx_mdp_gamma(void)
{
	...
	if (panel_type == PANEL_ID_VILLE_SONY_NT
		...
		mdp_color_enhancement(valente_wx_sony_nt_gamma, ARRAY_SIZE(valente_wx_sony_nt_gamma));
	}
#endif
	return 0;
}
...
#endif
	/*.mdp_color_enhance = valente_wx_mdp_color_enhance,*/

	.mdp_gamma = valente_wx_mdp_gamma,
...
static struct msm_fb_platform_data msm_fb_pdata = {
	.detect_client = valente_wx_detect_panel,
};
...
void __init valente_wx_allocate_fb_regions(void)
{
	void *addr;
	unsigned long size;
