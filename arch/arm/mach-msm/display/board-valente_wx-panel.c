...
#define GPIO_LCD_RSTz VALENTE_WX_GPIO_LCD_RSTz
#define GPIO_LCD_TE   VALENTE_WX_GPIO_LCD_TE
#else
#include "../board-valente_wx_c9.h"
#define GPIO_LCD_RSTz VALENTE_WX_C9_GPIO_LCD_RSTz
#define GPIO_LCD_TE   VALENTE_WX_C9_GPIO_LCD_TE
#endif
...
static void valente_wx_mipi_dsi_set_backlight(struct msm_fb_data_type *mfd)
{
	...
	if (mipi->mode == DSI_VIDEO_MODE) {
		...
		if (!mipi_lcd_on) {
			...
			if (panel_type == PANEL_ID_VALENTE_SAMSUNG_SG) {
				pr_info("[DISP] %s: PANEL_ID_VALENTE_WX_SAMSUNG_SG\n", __func__);
				...
				pr_info("[DISP] %s PANEL_ID_VALENTE_WX_SAMSUNG_SG_C2\n", __func__);
				cmdreq.cmds = samsung_cmd_on_cmds;
				cmdreq.cmds_cnt = ARRAY_SIZE(samsung_cmd_on_cmds);
			} else if (panel_type == PANEL_ID_VALENTE_SAMSUNG_SG_C3){
				pr_info("[DISP] %s: PANEL_ID_VALENTE_WX_SAMSUNG_SG_C3\n", __func__);
				cmdreq.cmds = samsung_cmd_on_cmds_c2;
				cmdreq.cmds_cnt = ARRAY_SIZE(samsung_cmd_on_cmds_c2);
			} else {
				pr_err("[DISP] %s: panel_type is not supported!(%d)\n", __func__, panel_type);
				cmdreq.cmds = samsung_cmd_on_cmds;
				cmdreq.cmds_cnt = ARRAY_SIZE(samsung_cmd_on_cmds);
			}
			cmdreq.flags = CMD_REQ_COMMIT;
...
static void valente_wx_set_backlight(struct msm_fb_data_type *mfd)
{
	...
	valente_wx_mipi_dsi_set_backlight(mfd);
...
static int mipi_dsi_panel_power(int on)
{
	...
	if (!dsi_power_on) {
		...
		rc = gpio_request(GPIO_LCD_RSTz, "LCM_RST_N");
		if (rc) {
			PR_DISP_ERR("%s:LCM gpio %d request failed, rc=%d\n", __func__,  GPIO_LCD_RSTz, rc);
			return -EINVAL;
		}
	...
	if (on) {
		...
		if (!mipi_lcd_on) {
			hr_msleep(10);
			gpio_set_value(GPIO_LCD_RSTz, 1);
			hr_msleep(1);
			gpio_set_value(GPIO_LCD_RSTz, 0);
			hr_msleep(35);
			gpio_set_value(GPIO_LCD_RSTz, 1);
		}
		hr_msleep(60);
