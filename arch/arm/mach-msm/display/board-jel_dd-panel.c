...
static struct dsi_buf jel_dd_panel_tx_buf;
static struct dsi_buf jel_dd_panel_rx_buf;
...
static struct msm_panel_common_pdata *mipi_jel_dd_pdata;
...
static DECLARE_WAIT_QUEUE_HEAD(jel_dd_vsync_wait);
static unsigned int vsync_irq;
static int wait_vsync = 0;
static int jel_dd_vsync_gpio = 0;
static int jel_dd_irq_cnt = 0;    /* irq trigger trace */

static irqreturn_t jel_dd_vsync_interrupt(int irq, void *data)
{
	jel_dd_irq_cnt++;
	if (wait_vsync) {
		PR_DISP_DEBUG("Wait vsync\n");
		jel_dd_vsync_gpio = 1;
		wake_up(&jel_dd_vsync_wait);
		wait_vsync = 0;
	}
...
static int setup_vsync(struct platform_device *pdev, int init)
{
	...
	ret = request_irq(vsync_irq, jel_dd_vsync_interrupt, IRQF_TRIGGER_RISING,
			  "vsync", pdev);
	if (ret)
...
static uint32 mipi_novatek_manufacture_id(struct msm_fb_data_type *mfd)
{
	...
	tp = &jel_dd_panel_tx_buf;
	rp = &jel_dd_panel_rx_buf;
...
static unsigned char jel_dd_shrink_pwm(int val)
{
...
#if defined CONFIG_FB_MSM_SELF_REFRESH
static void jel_dd_self_refresh_switch(int on)
{
	int vsync_timeout;
	...
	if (on) {
		...
		wait_vsync = 1;
		vsync_timeout = wait_event_timeout(jel_dd_vsync_wait, jel_dd_vsync_gpio ||
...
static void jel_dd_display_on(struct msm_fb_data_type *mfd)
{
...
static void jel_dd_mipi_dsi_set_backlight(struct msm_fb_data_type *mfd)
{
	...
	led_pwm1[1] = jel_dd_shrink_pwm(mfd->bl_level);
...
static int jel_dd_lcd_on(struct platform_device *pdev)
{
	...
	if (!mipi_lcd_on) {
		...
		gpio_set_value(jel_dd_LCD_RSTz, 0);
		hr_msleep(1);
		gpio_set_value(jel_dd_LCD_RSTz, 1);
		hr_msleep(20);
	}
...
static int jel_dd_lcd_off(struct platform_device *pdev)
{
	struct msm_fb_data_type *mfd;
...
static void jel_dd_set_backlight(struct msm_fb_data_type *mfd)
{
	int bl_level;
	...
	jel_dd_mipi_dsi_set_backlight(mfd);

}

static int __devinit jel_dd_lcd_probe(struct platform_device *pdev)
{
#if defined CONFIG_FB_MSM_SELF_REFRESH
	int ret;
	mipi_jel_dd_pdata = pdev->dev.platform_data;

	ret = setup_vsync(pdev, 1);
	if (ret) {
		dev_err(&pdev->dev, "mipi_jel_dd_setup_vsync failed\n");
		return ret;
	}
#endif
...
static void jel_dd_lcd_shutdown(struct platform_device *pdev)
{
	mipi_dsi_panel_power(0);
}

static struct platform_driver this_driver = {
	.probe  = jel_dd_lcd_probe,
	.shutdown = jel_dd_lcd_shutdown,
	.driver = {
		.name   = "mipi_novatek",
	},
};

static struct msm_fb_panel_data jel_dd_panel_data = {
	.on		= jel_dd_lcd_on,
	.off		= jel_dd_lcd_off,
	.set_backlight = jel_dd_set_backlight,
#if defined CONFIG_FB_MSM_SELF_REFRESH
	.self_refresh_switch = jel_dd_self_refresh_switch,
#endif
	.display_on = jel_dd_display_on,
#ifdef CONFIG_FB_MSM_CABC
...
int mipi_jel_dd_device_register(struct msm_panel_info *pinfo,
					u32 channel, u32 panel)
{
	...
	jel_dd_panel_data.panel_info = *pinfo;

	ret = platform_device_add_data(pdev, &jel_dd_panel_data,
		sizeof(jel_dd_panel_data));
	if (ret) {
...
static int mipi_cmd_novatek_blue_qhd_pt_init(void)
{
	...
	jel_dd_panel_data.self_refresh_switch = NULL;
#endif
	ret = mipi_jel_dd_device_register(&pinfo, MIPI_DSI_PRIM,
...
#ifdef CONFIG_FB_MSM_SELF_REFRESH
	jel_dd_panel_data.self_refresh_switch = NULL; /* CMD or VIDEO mode only */
#endif
	pinfo.lcd.vsync_enable = TRUE;
	pinfo.lcd.hw_vsync_mode = TRUE;
	pinfo.lcd.refx100 = 6096; /* adjust refx100 to prevent tearing */
...
static int mipi_dsi_panel_power(int on)
{
	static struct regulator *v_lcmio, *v_dsivdd;
	static bool bPanelPowerOn = false;
	int rc;

	char *lcmio_str = "8921_lvs5"; /*1.8V*/
	char *dsivdd_str = "8921_l2"; /* mipi 1.2 */

	PR_DISP_INFO("%s: state : %d\n", __func__, on);

	if (!dsi_power_on) {

		rc = gpio_request(jel_dd_NC_GPIO_1, "LCM_3V2_EN");
		if (rc) {
			PR_DISP_ERR("%s:LCM gpio %d request failed, rc=%d\n", __func__,  jel_dd_NC_GPIO_1, rc);
			return -EINVAL;
		}
		gpio_direction_output(jel_dd_NC_GPIO_1, 0);
		...
		rc = gpio_request(jel_dd_LCD_RSTz, "LCM_RST_N");
		if (rc) {
			PR_DISP_ERR("%s:LCM gpio %d request failed, rc=%d\n", __func__,  jel_dd_LCD_RSTz, rc);
			return -EINVAL;
		}
	...
	if (on) {
		...
		gpio_set_value(jel_dd_NC_GPIO_1, 1);
		...
		jel_dd_lcd_id_power(PM_GPIO_PULL_NO);
		...
		jel_dd_lcd_id_power(PM_GPIO_PULL_DN);
		...
		gpio_set_value(jel_dd_NC_GPIO_1, 0);
...
static struct mipi_dsi_platform_data mipi_dsi_pdata = {
	.vsync_gpio = jel_dd_LCD_TE,
	.dsi_power_save = mipi_dsi_panel_power,
};

static int __init jel_dd_panel_init(void)
{
	...
	if (panel_type == PANEL_ID_NONE) {
		if (board_mfg_mode() == 8 || board_mfg_mode() == 4) {
			PR_DISP_INFO("%s: enter mfgkernel/powertest \n", __func__);
			return 0;
...
static int __init jel_dd_panel_late_init(void)
{
	mipi_dsi_buf_alloc(&jel_dd_panel_tx_buf, DSI_BUF_SIZE);
	mipi_dsi_buf_alloc(&jel_dd_panel_rx_buf, DSI_BUF_SIZE);
...
module_init(jel_dd_panel_init);
late_initcall(jel_dd_panel_late_init);
