...
static struct dsi_buf evita_utl_panel_tx_buf;
static struct dsi_buf evita_utl_panel_rx_buf;
...
static struct msm_panel_common_pdata *mipi_evita_utl_pdata;
...
static DECLARE_WAIT_QUEUE_HEAD(evita_utl_vsync_wait);
static unsigned int vsync_irq;
static int wait_vsync = 0;
static int evita_utl_vsync_gpio = 0;
static int evita_utl_irq_cnt = 0;    /* irq trigger trace */

static irqreturn_t evita_utl_vsync_interrupt(int irq, void *data)
{
	evita_utl_irq_cnt++;
	if (wait_vsync) {
		PR_DISP_DEBUG("Wait vsync\n");
		evita_utl_vsync_gpio = 1;
		wake_up(&evita_utl_vsync_wait);
		wait_vsync = 0;
	}
...
static int setup_vsync(struct platform_device *pdev, int init)
{
	...
	ret = request_irq(vsync_irq, evita_utl_vsync_interrupt, IRQF_TRIGGER_RISING,
			  "vsync", pdev);
	if (ret) {
...
static struct dsi_cmd_desc evita_utl_manufacture_id_cmd = {
	DTYPE_DCS_READ, 1, 0, 1, 5, sizeof(manufacture_id), manufacture_id};

static uint32 mipi_evita_utl_manufacture_id(struct msm_fb_data_type *mfd)
{
	struct dsi_buf *rp, *tp;
	struct dsi_cmd_desc *cmd;
	uint32 *lp;

	tp = &evita_utl_panel_tx_buf;
	rp = &evita_utl_panel_rx_buf;
	mipi_dsi_buf_init(rp);
	mipi_dsi_buf_init(tp);

	cmd = &evita_utl_manufacture_id_cmd;
...
static struct dsi_cmd_desc *evita_utl_video_on_cmds = NULL;
static struct dsi_cmd_desc *evita_utl_display_off_cmds = NULL;
static struct dsi_cmd_desc *evita_utl_cmd_backlight_cmds = NULL;
#if defined CONFIG_FB_MSM_SELF_REFRESH
static struct dsi_cmd_desc *evita_utl_video_to_cmd = NULL;
static struct dsi_cmd_desc *evita_utl_cmd_to_video = NULL;
int evita_utl_video_to_cmd_cnt = 0;
int evita_utl_cmd_to_video_cnt = 0;
#endif
int evita_utl_video_on_cmds_count = 0;
int evita_utl_display_off_cmds_count = 0;
int evita_utl_cmd_backlight_cmds_count = 0;
...
static unsigned char evita_utl_shrink_pwm(int val)
{
...
#if defined CONFIG_FB_MSM_SELF_REFRESH
static void evita_utl_self_refresh_switch(int on)
{
	int vsync_timeout;
	...
	if (on) {
		...
		if (panel_type != PANEL_ID_ELITE_SHARP_HX) {
			wait_vsync = 1;
			vsync_timeout = wait_event_timeout(evita_utl_vsync_wait, evita_utl_vsync_gpio ||
			...
			evita_utl_vsync_gpio = 0;
			wait_vsync = 1;
			vsync_timeout = wait_event_timeout(evita_utl_vsync_wait, evita_utl_vsync_gpio ||
						gpio_get_value(0), HZ/2);
			if (evita_utl_irq_cnt > 2)
...
static void evita_utl_display_on(struct msm_fb_data_type *mfd)
{
...
static void evita_utl_mipi_dsi_set_backlight(struct msm_fb_data_type *mfd)
{
	...
	led_pwm1[1] = evita_utl_shrink_pwm(mfd->bl_level);
	...
	cmdreq.cmds = evita_utl_cmd_backlight_cmds;
	cmdreq.cmds_cnt = 1;
	cmdreq.flags = CMD_REQ_COMMIT;
	cmdreq.rlen = 0;
	cmdreq.cb = NULL;
...
static int evita_utl_lcd_on(struct platform_device *pdev)
{
	...
	if (panel_type != PANEL_ID_ELITE_SHARP_HX) {
		if (!mipi_lcd_on) {
			...
			gpio_set_value(EVITA_UTL_GPIO_LCD_RSTz, 0);
			hr_msleep(1);
			gpio_set_value(EVITA_UTL_GPIO_LCD_RSTz, 1);
			hr_msleep(20);
		}
	}
	...
	if (mipi->mode == DSI_VIDEO_MODE) {
		if(!mipi_lcd_on) {
			if (panel_type == PANEL_ID_ELITE_SONY_NT
				...
				cmdreq.cmds = evita_utl_video_on_cmds;
				cmdreq.cmds_cnt = evita_utl_video_on_cmds_count;
				cmdreq.flags = CMD_REQ_COMMIT;
				cmdreq.rlen = 0;
				cmdreq.cb = NULL;
				mipi_dsi_cmdlist_put(&cmdreq);
		...
		mipi_dsi_cmd_bta_sw_trigger(); /* clean up ack_err_status */
		mipi_evita_utl_manufacture_id(mfd);
	}
...
static int evita_utl_lcd_off(struct platform_device *pdev)
{
	struct msm_fb_data_type *mfd;
	...
	if (panel_type != PANEL_ID_NONE) {
		...
		cmdreq.cmds = evita_utl_display_off_cmds;
		cmdreq.cmds_cnt = evita_utl_display_off_cmds_count;
		cmdreq.flags = CMD_REQ_COMMIT;
		cmdreq.rlen = 0;
		cmdreq.cb = NULL;
		mipi_dsi_cmdlist_put(&cmdreq);
...
static void evita_utl_set_backlight(struct msm_fb_data_type *mfd)
{
	...
	evita_utl_mipi_dsi_set_backlight(mfd);

}

static int __devinit evita_utl_lcd_probe(struct platform_device *pdev)
{
#if defined CONFIG_FB_MSM_SELF_REFRESH
	int ret;
	PR_DISP_DEBUG("[SR]%s pdev->id=%d\n", __func__, pdev->id);
	mipi_evita_utl_pdata = pdev->dev.platform_data;

	ret = setup_vsync(pdev, 1);
	if (ret) {
		dev_err(&pdev->dev, "mipi_evita_utl_setup_vsync failed\n");
		return ret;
	}
#endif
...
static void evita_utl_lcd_shutdown(struct platform_device *pdev)
{
...
static struct platform_driver this_driver = {
	.probe  = evita_utl_lcd_probe,
	.shutdown = evita_utl_lcd_shutdown,
	.driver = {
		.name   = "mipi_evita_utl",
	},
};

static struct msm_fb_panel_data evita_utl_panel_data = {
	.on		= evita_utl_lcd_on,
	.off		= evita_utl_lcd_off,
	.set_backlight = evita_utl_set_backlight,
#if defined CONFIG_FB_MSM_SELF_REFRESH
	.self_refresh_switch = evita_utl_self_refresh_switch,
#endif
	.display_on = evita_utl_display_on,
#ifdef CONFIG_FB_MSM_CABC
...
int mipi_evita_utl_device_register(struct msm_panel_info *pinfo,
					u32 channel, u32 panel)
{
	...
	pdev = platform_device_alloc("mipi_evita_utl", (panel << 8)|channel);
	if (!pdev)
		return -ENOMEM;

	evita_utl_panel_data.panel_info = *pinfo;

	ret = platform_device_add_data(pdev, &evita_utl_panel_data,
		sizeof(evita_utl_panel_data));
	if (ret) {
...
static int mipi_video_sony_hd720p_init(void)
{
	...
	/* 1:VIDEO MODE, 0:CMD MODE */
		...
		evita_utl_panel_data.self_refresh_switch = NULL; /* CMD or VIDEO mode only */
#endif
	...
	ret = mipi_evita_utl_device_register(&pinfo, MIPI_DSI_PRIM, MIPI_DSI_PANEL_WVGA_PT);
	if (ret)
	...
	if (panel_type == PANEL_ID_ELITE_SONY_NT) {
		...
		evita_utl_video_on_cmds = sony_panel_video_mode_cmds_id18103_ver008;
		evita_utl_video_on_cmds_count = ARRAY_SIZE(sony_panel_video_mode_cmds_id18103_ver008);
		...
		evita_utl_video_on_cmds = sony_panel_video_mode_cmds_id28103_ver008;
		evita_utl_video_on_cmds_count = ARRAY_SIZE(sony_panel_video_mode_cmds_id28103_ver008);
		...
		evita_utl_video_on_cmds = sony_panel_video_mode_cmds_c2;
		evita_utl_video_on_cmds_count = ARRAY_SIZE(sony_panel_video_mode_cmds_c2);
	}

	evita_utl_display_off_cmds	= sony_display_off_cmds;
	evita_utl_cmd_backlight_cmds = sony_cmd_backlight_cmds;
#if defined CONFIG_FB_MSM_SELF_REFRESH
	evita_utl_video_to_cmd = video_to_cmd;
	evita_utl_cmd_to_video = cmd_to_video;
#endif

	evita_utl_display_off_cmds_count = ARRAY_SIZE(sony_display_off_cmds);
	evita_utl_cmd_backlight_cmds_count = ARRAY_SIZE(sony_cmd_backlight_cmds);
#if defined CONFIG_FB_MSM_SELF_REFRESH
	evita_utl_video_to_cmd_cnt	= ARRAY_SIZE(video_to_cmd);
	evita_utl_cmd_to_video_cnt = ARRAY_SIZE(cmd_to_video);
#endif
	return ret;
}
...
static int __init mipi_video_himax_720p_pt_init(void)
{
	...
	if(0)
	evita_utl_video_on_cmds = himax_video_on_cmds;
	evita_utl_video_on_cmds = himax_video_on_cmds_id311100;
	evita_utl_display_off_cmds	= himax_display_off_cmds;
	evita_utl_cmd_backlight_cmds = himax_cmd_backlight_cmds;
#if defined CONFIG_FB_MSM_SELF_REFRESH
	evita_utl_video_to_cmd = video_to_cmd;
	evita_utl_cmd_to_video = cmd_to_video;
#endif
	...
	if(0)
	evita_utl_video_on_cmds_count = ARRAY_SIZE(himax_video_on_cmds);
	evita_utl_video_on_cmds_count = ARRAY_SIZE(himax_video_on_cmds_id311100);
	evita_utl_display_off_cmds_count = ARRAY_SIZE(himax_display_off_cmds);
	evita_utl_cmd_backlight_cmds_count = ARRAY_SIZE(himax_cmd_backlight_cmds);
#if defined CONFIG_FB_MSM_SELF_REFRESH
	evita_utl_video_to_cmd_cnt	= ARRAY_SIZE(video_to_cmd);
	evita_utl_cmd_to_video_cnt = ARRAY_SIZE(cmd_to_video);
#endif
...
static int __init mipi_video_sharp_nt_720p_pt_init(void)
{
	...
	if (panel_type == PANEL_ID_ELITE_SHARP_NT) {
		...
		evita_utl_video_on_cmds = sharp_nt_video_on_cmds_idA1B100;
		evita_utl_video_on_cmds_count = ARRAY_SIZE(sharp_nt_video_on_cmds_idA1B100);
		...
		evita_utl_video_on_cmds = sharp_nt_video_on_cmds_nv3;
		evita_utl_video_on_cmds_count = ARRAY_SIZE(sharp_nt_video_on_cmds_nv3);
		...
		evita_utl_video_on_cmds = sharp_nt_video_on_cmds_nv4;
		evita_utl_video_on_cmds_count = ARRAY_SIZE(sharp_nt_video_on_cmds_nv4);
	}
...
uint32_t cfg_panel_te_active[] = {GPIO_CFG(EVITA_UTL_GPIO_LCD_TE, 1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA)};
uint32_t cfg_panel_te_sleep[] = {GPIO_CFG(EVITA_UTL_GPIO_LCD_TE, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA)};
...
static int mipi_dsi_panel_power(int on)
{
	...
	if (!dsi_power_on) {
		...
		rc = gpio_request(EVITA_UTL_GPIO_LCD_RSTz, "LCM_RST_N");
		if (rc) {
			PR_DISP_ERR("%s:LCM gpio %d request failed, rc=%d\n", __func__,  EVITA_UTL_GPIO_LCD_RSTz, rc);
			return -EINVAL;
		}
	...
	if (on) {
		...
		evita_utl_lcd_id_power(PM_GPIO_PULL_NO);
		...
		evita_utl_lcd_id_power(PM_GPIO_PULL_DN);
...
static struct mipi_dsi_platform_data mipi_dsi_pdata = {
	.vsync_gpio = EVITA_UTL_GPIO_LCD_TE,
	.dsi_power_save = mipi_dsi_panel_power,
	.splash_is_enabled = mipi_dsi_splash_is_enabled,
};

static int __init evita_utl_panel_init(void)
{
	if(panel_type != PANEL_ID_NONE)
...
static int __init evita_utl_panel_late_init(void)
{
	mipi_dsi_buf_alloc(&evita_utl_panel_tx_buf, DSI_BUF_SIZE);
	mipi_dsi_buf_alloc(&evita_utl_panel_rx_buf, DSI_BUF_SIZE);
...
module_init(evita_utl_panel_init);
late_initcall(evita_utl_panel_late_init);
