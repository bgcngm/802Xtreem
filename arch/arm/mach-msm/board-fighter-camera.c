...
//TODO: FIXME #ifdef CONFIG_MSM_CAMERA
...
static int fighter_use_ext_1v2(void)
{
...
#define FIGHTER_V_RAW_1V8_EN PM8921_GPIO_PM_TO_SYS(FIGHTER_PMGPIO_RAW_1V8_EN)
#define FIGHTER_V_RAW_1V2_EN PM8921_GPIO_PM_TO_SYS(FIGHTER_PMGPIO_RAW_1V2_EN)
static int fighter_rawchip_vreg_on(void)
{
	int rc;
	pr_info("%s\n", __func__);

	/* PM8921_GPIO_PM_TO_SYS(FIGHTER_PMGPIO_RAW_1V8_EN) 1800000 */
	rc = gpio_request(FIGHTER_V_RAW_1V8_EN, "V_RAW_1V8_EN");
	if (rc) {
		pr_err("rawchip on\
			(\"gpio %d\", 1.2V) FAILED %d\n",
			FIGHTER_V_RAW_1V8_EN, rc);
		goto enable_1v8_fail;
	}
	gpio_direction_output(FIGHTER_V_RAW_1V8_EN, 1);
	gpio_free(FIGHTER_V_RAW_1V8_EN);

	mdelay(5);

	/* PM8921_GPIO_PM_TO_SYS(FIGHTER_PMGPIO_RAW_1V2_EN) 1200000 */
	if (system_rev < 2) {	/*merge GPIO43 to GPIO95 for XC */
		rc = gpio_request(FIGHTER_V_RAW_1V2_EN, "V_RAW_1V2_EN");
		if (rc) {
			pr_err("rawchip on\
			(\"gpio %d\", 1.2V) FAILED %d\n",
			FIGHTER_V_RAW_1V2_EN, rc);
			goto enable_1v2_fail;
		}
		gpio_direction_output(FIGHTER_V_RAW_1V2_EN, 1);
		gpio_free(FIGHTER_V_RAW_1V2_EN);
	}

	if (fighter_use_ext_1v2()) { /* use external 1v2 for HW workaround */
		mdelay(1);

		rc = gpio_request(FIGHTER_GPIO_V_CAM_D1V2_EN, "rawchip");
		pr_info("rawchip external 1v2 gpio_request,%d\n",
		FIGHTER_GPIO_V_CAM_D1V2_EN);
		if (rc < 0) {
			pr_err("GPIO(%d) request failed", FIGHTER_GPIO_V_CAM_D1V2_EN);
			goto enable_1v2_fail;
		}
		gpio_direction_output(FIGHTER_GPIO_V_CAM_D1V2_EN, 1);
		gpio_free(FIGHTER_GPIO_V_CAM_D1V2_EN);
		mdelay(1);
	}
	return rc;
...
enable_1v2_fail:
	...
	if (rc)
		pr_err("rawchip on\
			(\"gpio %d\", 1.2V) FAILED %d\n",
			FIGHTER_V_RAW_1V8_EN, rc);
	gpio_direction_output(FIGHTER_V_RAW_1V8_EN, 0);
	gpio_free(FIGHTER_V_RAW_1V8_EN);
enable_1v8_fail:
	return rc;
}

static int fighter_rawchip_vreg_off(void)
{
	int rc = 0;
	...
	if (fighter_use_ext_1v2()) { /* use external 1v2 for HW workaround */
		...
		if (rc)
			pr_err("rawchip off(\
				\"gpio %d\", 1.2V) FAILED %d\n",
				FIGHTER_GPIO_V_CAM_D1V2_EN, rc);
		gpio_direction_output(FIGHTER_GPIO_V_CAM_D1V2_EN, 0);
		gpio_free(FIGHTER_GPIO_V_CAM_D1V2_EN);
	...
	if (system_rev < 2) {	/*merge GPIO43 to GPIO95 for XC */
		...
		\"gpio %d\", 1.2V) FAILED %d\n",
		FIGHTER_V_RAW_1V2_EN, rc);
		gpio_direction_output(FIGHTER_V_RAW_1V2_EN, 0);
		gpio_free(FIGHTER_V_RAW_1V2_EN);
...
static struct msm_camera_rawchip_info msm_rawchip_board_info = {
	.rawchip_reset	= FIGHTER_GPIO_RAW_RSTN,
	.rawchip_intr0	= MSM_GPIO_TO_INT(FIGHTER_GPIO_RAW_INTR0),
	.rawchip_intr1	= MSM_GPIO_TO_INT(FIGHTER_GPIO_RAW_INTR1),
	...
	.camera_rawchip_power_on = fighter_rawchip_vreg_on,
	.camera_rawchip_power_off = fighter_rawchip_vreg_off,
	.rawchip_use_ext_1v2 = fighter_use_ext_1v2,
};
...
struct msm_camera_board_info fighter_camera_board_info_2nd = {
