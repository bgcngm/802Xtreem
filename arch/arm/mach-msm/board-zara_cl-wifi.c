/* linux/arch/arm/mach-msm/board-zara_cl-wifi.c
*/
...
#include "board-zara_cl-wifi.h"

int zara_cl_wifi_power(int on);
int zara_cl_wifi_reset(int on);
int zara_cl_wifi_set_carddetect(int on);
int zara_cl_wifi_get_mac_addr(unsigned char *buf);
...
static void *zara_cl_wifi_mem_prealloc(int section, unsigned long size)
{
...
int __init zara_cl_init_wifi_mem(void)
{
	int i;
...
static struct resource zara_cl_wifi_resources[] = {
	[0] = {
...
static struct wifi_platform_data zara_cl_wifi_control = {
	.set_power      = zara_cl_wifi_power,
	.set_reset      = zara_cl_wifi_reset,
	.set_carddetect = zara_cl_wifi_set_carddetect,
	.mem_prealloc   = zara_cl_wifi_mem_prealloc,
	.get_mac_addr	= zara_cl_wifi_get_mac_addr,
	.bus_scale_table    = &wlan_bus_scale_pdata,
};

static struct platform_device zara_cl_wifi_device = {
	.name           = "bcmdhd_wlan",
	.id             = 1,
	.num_resources  = ARRAY_SIZE(zara_cl_wifi_resources),
	.resource       = zara_cl_wifi_resources,
	.dev            = {
		.platform_data = &zara_cl_wifi_control,
	},
};

static unsigned zara_cl_wifi_update_nvs(char *str)
{
...
#else
	zara_cl_wifi_update_nvs("sd_oobonly=1\n");
#endif
	zara_cl_wifi_update_nvs("btc_params80=0\n");
	zara_cl_wifi_update_nvs("btc_params6=30\n");
	zara_cl_init_wifi_mem();
	ret = platform_device_register(&zara_cl_wifi_device);
	return ret;
}
