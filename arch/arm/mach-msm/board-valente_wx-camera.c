...
/* HTC_START Turn off backlight when flash on */
	...
	pr_info("%s, linear led, mode %d backlight_off %d", __func__, mode, backlight_off);
	...
	if (mode != FL_MODE_PRE_FLASH && mode != FL_MODE_OFF) {
		if (!backlight_off) {
			...
			if (brightness >= 0 && brightness <= 255) {
				pr_info("%s, Turn off backlight before flashlight, brightness %d", __func__, brightness);
				led_brightness_value_set("lcd-backlight", 0);
				backlight_off = 1;
			} else
				pr_err("%s, Invalid brightness value!! brightness %d", __func__, brightness);
		}
	}
	...
	if (mode == FL_MODE_PRE_FLASH || mode == FL_MODE_OFF) {
		if(backlight_off) {
			pr_info("%s, Turn on backlight after flashlight, brightness %d", __func__, brightness);
...
#define VALENTE_WX_SYS_CAM_PWDN PM8921_GPIO_PM_TO_SYS(VALENTE_WX_PMGPIO_CAM_PWDN)
