/* linux/arch/arm/mach-msm/board-k2_plc_cl-mmc.c
 *
 ...
 */
static struct embedded_sdio_data k2_plc_cl_wifi_emb_data = {
	.cccr	= {
		.sdio_vsn	= 2,
		.multi_block	= 1,
		.low_speed	= 0,
...
static int
k2_plc_cl_wifi_status_register(void (*callback)(int card_present, void *dev_id),
				void *dev_id)
{
	if (wifi_status_cb)
		return -EAGAIN;
...
static int k2_plc_cl_wifi_cd;	/* WiFi virtual 'card detect' status */

static unsigned int k2_plc_cl_wifi_status(struct device *dev)
{
	return k2_plc_cl_wifi_cd;
}

static unsigned int k2_plc_cl_wifislot_type = MMC_TYPE_SDIO_WIFI;
static unsigned int wifi_sup_clk_rates[] = {
	400000, 24000000, 48000000,
};
static struct mmc_platform_data k2_plc_cl_wifi_data = {
	.ocr_mask               = MMC_VDD_28_29,
	.status                 = k2_plc_cl_wifi_status,
	.register_status_notify = k2_plc_cl_wifi_status_register,
	.embedded_sdio          = &k2_plc_cl_wifi_emb_data,
	.mmc_bus_width  = MMC_CAP_4_BIT_DATA,
	.sup_clk_table	= wifi_sup_clk_rates,
	.sup_clk_cnt	= ARRAY_SIZE(wifi_sup_clk_rates),
	.slot_type = &k2_plc_cl_wifislot_type,
...
int k2_plc_cl_wifi_set_carddetect(int val)
{
	printk(KERN_INFO "%s: %d\n", __func__, val);
	k2_plc_cl_wifi_cd = val;
	if (wifi_status_cb)
...
EXPORT_SYMBOL(k2_plc_cl_wifi_set_carddetect);
...
EXPORT_SYMBOL(k2_plc_cl_wifi_power);
