#if 0
/* linux/arch/arm/mach-msm/t6china-wifi.c
*/
...
#include "board-t6china-wifi.h"

int t6china_wifi_power(int on);
int t6china_wifi_reset(int on);
int t6china_wifi_set_carddetect(int on);
int t6china_wifi_get_mac_addr(unsigned char *buf);
...
static void *t6china_wifi_mem_prealloc(int section, unsigned long size)
{
...
int __init t6china_init_wifi_mem(void)
{
	int i;
...
static struct resource t6china_wifi_resources[] = {
	[0] = {
...
static struct wifi_platform_data t6china_wifi_control = {
	.set_power      = t6china_wifi_power,
	.set_reset      = t6china_wifi_reset,
	.set_carddetect = t6china_wifi_set_carddetect,
	.mem_prealloc   = t6china_wifi_mem_prealloc,
	.get_mac_addr	= t6china_wifi_get_mac_addr,
};

static struct platform_device t6china_wifi_device = {
	.name           = "bcmdhd_wlan",
	.id             = 1,
	.num_resources  = ARRAY_SIZE(t6china_wifi_resources),
	.resource       = t6china_wifi_resources,
	.dev            = {
		.platform_data = &t6china_wifi_control,
	},
};

static unsigned t6china_wifi_update_nvs(char *str)
{
...
#else
	t6china_wifi_update_nvs("sd_oobonly=1\n");
#endif
	t6china_wifi_update_nvs("btc_params80=0\n");
	t6china_wifi_update_nvs("btc_params6=30\n");
	t6china_init_wifi_mem();
	ret = platform_device_register(&t6china_wifi_device);
	return ret;
}
#endif
