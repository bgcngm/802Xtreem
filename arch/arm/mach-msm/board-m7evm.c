...
static struct platform_device m7evm_android_pmem_device = {
	.name = "android_pmem",
	.id = 0,
...
static struct platform_device m7evm_android_pmem_adsp_device = {
	.name = "android_pmem",
	.id = 2,
	.dev = { .platform_data = &android_pmem_adsp_pdata },
};

static struct android_pmem_platform_data m7evm_android_pmem_audio_pdata = {
	.name = "pmem_audio",
	.allocator_type = PMEM_ALLOCATORTYPE_BITMAP,
	.cached = 0,
	.memory_type = MEMTYPE_EBI1,
...
static struct msm_rtb_platform_data m7evm_rtb_pdata = {
...
static int __init msm_rtb_set_buffer_size(char *p)
{
       ...
       s = memparse(p, NULL);
       m7evm_rtb_pdata.size = ALIGN(s, SZ_4K);
       return 0;
}
early_param("msm_rtb_size", msm_rtb_set_buffer_size);


static struct platform_device m7evm_rtb_device = {
       .name           = "msm_rtb",
       .id             = -1,
       .dev            = {
               .platform_data = &m7evm_rtb_pdata,
       },
};
#endif
...
static int m7evm_paddr_to_memtype(unsigned int paddr)
{
	return MEMTYPE_EBI1;
}
...
#ifdef CONFIG_MSM_MULTIMEDIA_USE_ION
static struct ion_cp_heap_pdata cp_mm_m7evm_ion_pdata = {
...
static struct ion_cp_heap_pdata cp_mfc_m7evm_ion_pdata = {
...
static struct ion_co_heap_pdata co_m7evm_ion_pdata = {
	.adjacent_mem_id = INVALID_HEAP_ID,
	.align = PAGE_SIZE,
	.mem_is_fmem = 0,
};

static struct ion_co_heap_pdata fw_co_m7evm_ion_pdata = {
...
static struct ion_platform_data ion_pdata = {
	...
	.heaps = {
...
#ifdef CONFIG_MSM_MULTIMEDIA_USE_ION
		{
			...
			.extra_data = (void *) &cp_mm_m7evm_ion_pdata,
		},
		{
			...
			.memory_type = ION_EBI_TYPE,
			.extra_data = (void *) &fw_co_m7evm_ion_pdata,
		},
		{
			...
			.memory_type = ION_EBI_TYPE,
			.extra_data = (void *) &cp_mfc_m7evm_ion_pdata,
		},
#ifndef CONFIG_MSM_IOMMU
		{
			...
			.memory_type = ION_EBI_TYPE,
			.extra_data = (void *) &co_m7evm_ion_pdata,
		},
#endif
		{
...
static struct platform_device m7evm_ion_dev = {
	.name = "ion-msm",
	.id = 1,
...
static void __init m7evm_calculate_reserve_sizes(void)
{
	size_pmem_devices();
	reserve_pmem_memory();
	reserve_ion_memory();
...
static struct reserve_info m7evm_reserve_info __initdata = {
	.memtype_reserve_table = apq8064_reserve_table,
	.calculate_reserve_sizes = m7evm_calculate_reserve_sizes,
	.reserve_fixed_area = apq8064_reserve_fixed_area,
	.paddr_to_memtype = m7evm_paddr_to_memtype,
};

static int m7evm_memory_bank_size(void)
{
	return 1<<29;
}
...
static void __init locate_unstable_memory(void)
{
	...
	bank_size = m7evm_memory_bank_size();
	...
	m7evm_reserve_info.low_unstable_address = mb->start -
					MIN_MEMORY_BLOCK_SIZE + mb->size;
	m7evm_reserve_info.max_unstable_size = MIN_MEMORY_BLOCK_SIZE;

	m7evm_reserve_info.bank_size = bank_size;
	pr_info("low unstable address %lx max size %lx bank size %lx\n",
		m7evm_reserve_info.low_unstable_address,
		m7evm_reserve_info.max_unstable_size,
		m7evm_reserve_info.bank_size);
	return;
#endif
no_dmm:
	m7evm_reserve_info.low_unstable_address = high;
	m7evm_reserve_info.max_unstable_size = 0;
}

int __init parse_tag_memsize(const struct tag *tags);
static unsigned int mem_size_mb;

static void __init m7evm_reserve(void)
{
	if (mem_size_mb == 64)
		return;
...
static void __init m7evm_early_reserve(void)
{
	reserve_info = &m7evm_reserve_info;
...
static void m7evm_usb_dpdn_switch(int path)
{
	switch (path) {
	case PATH_USB:
...
static T_MHL_PLATFORM_DATA mhl_sii9234_device_data = {
	...
	.ci2ca = 0,
#ifdef CONFIG_FB_MSM_HDMI_MHL
	.mhl_usb_switch		= m7evm_usb_dpdn_switch,
...
static struct android_usb_platform_data android_usb_pdata = {
	...
	.fserial_init_string = "HSIC:modem,HSUART:modem,tty,tty:autobot,tty:serial,tty:autobot",
...
static int64_t m7evm_get_usbid_adc(void)
{
...
static void m7evm_config_usb_id_gpios(bool output)
{
	int rc;
...
static struct cable_detect_platform_data cable_detect_pdata = {
       ...
       .usb_id_pin_gpio        = USB1_HS_ID_GPIO,
       .get_adc_cb             = m7evm_get_usbid_adc,
       .config_usb_id_gpios    = m7evm_config_usb_id_gpios,
#if 0
       .mhl_1v2_power = mhl_sii9234_1v2_power,
       .usb_dpdn_switch        = m7evm_usb_dpdn_switch,
#endif
};
...
void m7evm_cable_detect_register(void)
{
	int rc;
...
void m7evm_pm8xxx_adc_device_register(void)
{
	pr_info("%s: Register PM8XXX ADC device. rev: %d\n",
		__func__, system_rev);
	m7evm_cable_detect_register();
}

void m7evm_add_usb_devices(void)
{
...
static struct wcd9xxx_pdata m7evm_tabla_platform_data = {
...
static struct slim_device m7evm_slim_tabla = {
	.name = "tabla-slim",
	.e_addr = {0, 1, 0x10, 0, 0x17, 2},
	.dev = {
		.platform_data = &m7evm_tabla_platform_data,
	},
};

static struct wcd9xxx_pdata m7evm_tabla20_platform_data = {
	.slimbus_slave_device = {
		.name = "tabla-slave",
...
static struct slim_device m7evm_slim_tabla20 = {
	.name = "tabla2x-slim",
	.e_addr = {0, 1, 0x60, 0, 0x17, 2},
	.dev = {
		.platform_data = &m7evm_tabla20_platform_data,
	},
};
...
static struct synaptics_i2c_rmi_platform_data syn_ts_3k_data[] = { 
	{
		...
		.config = {0x33, 0x32, 0x00, 0x03, 0x00, 0x7F, 0x03, 0x1E,
			...
			0x13, 0x04, 0x1B, 0x00, 0x10, 0x0A, 0x80, 0x80,
	...
	},
	{
		...
		.reduce_report_level = {60, 60, 50, 0, 0},
		.virtual_key = m7_vk_data,
		.config = {0x33, 0x32, 0x01, 0x03, 0x00, 0x7F, 0x03, 0x1E,
			0x14, 0x09, 0x00, 0x01, 0x01, 0x00, 0x10, 0x54,
			0x06, 0x40, 0x0B, 0x02, 0x14, 0x1E, 0x05, 0x4B,
			0x26, 0x2E, 0x6F, 0x01, 0x01, 0x3C, 0x0C, 0x03,
			0x10, 0x03, 0x29, 0x44, 0xC3, 0x45, 0x50, 0xC3,
	...
	},
	{
		...
		.reduce_report_level = {60, 60, 50, 0, 0},
		.virtual_key = m7_vk_data,
		.config = {0x33, 0x32, 0x02, 0x03, 0x00, 0x7F, 0x03, 0x1E,
			0x14, 0x09, 0x00, 0x01, 0x01, 0x00, 0x10, 0x54,
			0x06, 0x40, 0x0B, 0x02, 0x14, 0x1E, 0x05, 0x4B,
			0x26, 0x2E, 0x6F, 0x01, 0x01, 0x3C, 0x0C, 0x03,
			0x10, 0x03, 0x29, 0x44, 0xC3, 0x45, 0x50, 0xC3,
...
{
	return sprintf(buf,
		__stringify(EV_KEY) ":" __stringify(KEY_BACK)       ":105:1340:133:107"
		":" __stringify(EV_KEY) ":" __stringify(KEY_HOME)   ":360:1340:133:107"
		"\n");
...
static struct cm3629_platform_data cm36282_pdata_r8 = {
	...
	.ps_select = CM3629_PS1_ONLY,
	.intr = PM8921_GPIO_PM_TO_SYS(PROXIMITY_INT),
	.levels = { 12, 14, 44, 242, 608, 2162, 3688, 6589, 9491, 65535},
        .golden_adc = 0xA7D,
	.power = NULL,
	.cm3629_slave_address = 0xC0>>1,
	.ps1_thd_set = 0x15,
...
static void __init m7evm_map_io(void)
{
...
static void __init m7evm_init_irq(void)
{
...
static void __init m7evm_init_buses(void)
{
...
static mhl_driving_params m7evm_driving_params[] = {
...
static struct msm_hdmi_platform_data hdmi_msm_data = {
	...
	.driving_params = m7evm_driving_params,
	.dirving_params_count = ARRAY_SIZE(m7evm_driving_params),
};
...
#ifdef CONFIG_ANDROID_PMEM
#ifndef CONFIG_MSM_MULTIMEDIA_USE_ION
	&m7evm_android_pmem_device,
	&m7evm_android_pmem_adsp_device,
	&m7evm_android_pmem_audio_device,
#endif 
#endif 
#ifdef CONFIG_ION_MSM
	&m7evm_ion_dev,
...
static struct msm_spi_platform_data m7evm_qup_spi_gsbi5_pdata = {
...
static struct slim_boardinfo m7evm_slim_devices[] = {
	{
		.bus_num = 1,
		.slim_slave = &m7evm_slim_tabla,
	},
	{
		.bus_num = 1,
		.slim_slave = &m7evm_slim_tabla20,
	},
...
static struct msm_i2c_platform_data m7evm_i2c_qup_gsbi3_pdata = {
	.clk_freq = 400000,
	.src_clk_rate = 24000000,
...
static struct msm_i2c_platform_data m7evm_i2c_qup_gsbi4_pdata = {
	.clk_freq = 400000,
	.src_clk_rate = 24000000,
	.share_uart_flag = 1,
};

static struct msm_i2c_platform_data m7evm_i2c_qup_gsbi7_pdata = {
	.clk_freq = 400000,
	.src_clk_rate = 24000000,
	.share_uart_flag = 1,
};
...
#endif
static struct cir_platform_data m7evm_cir_gsbi3_pdata = {
...
static void __init m7evm_i2c_init(void)
{
	...
	apq8064_device_qup_i2c_gsbi3.dev.platform_data =
					&m7evm_i2c_qup_gsbi3_pdata;
	apq8064_device_qup_i2c_gsbi4.dev.platform_data =
					&m7evm_i2c_qup_gsbi4_pdata;
	apq8064_device_qup_i2c_gsbi7.dev.platform_data =
					&m7evm_i2c_qup_gsbi7_pdata;
}
...
static void __init m7evm_init_dsps(void)
{
...
static int m7evm_mpu3050_sensor_power_LPM(int on)
{
	int rc = 0;
...
static struct mpu3050_platform_data mpu3050_data = {
	...
	.power_LPM = m7evm_mpu3050_sensor_power_LPM,
};
...
static struct i2c_registry m7evm_i2c_devices[] __initdata = {
	{
		I2C_SURF | I2C_FFA,
...
static void __init register_i2c_devices(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(m7evm_i2c_devices); ++i) {
		if (m7evm_i2c_devices[i].machs & mach_mask) {
			i2c_register_board_info(m7evm_i2c_devices[i].bus,
						m7evm_i2c_devices[i].info,
						m7evm_i2c_devices[i].len);
		}
	}
...
static void __init m7evm_receiver_init(void)
{
...
static void __init m7evm_cir_init(void)
{
	...
	apq8064_device_uart_gsbi3.dev.platform_data =
					&m7evm_cir_gsbi3_pdata;
}
#endif

static void __init m7evm_common_init(void)
{
	int rc = 0;
	...
	slim_register_board_info(m7evm_slim_devices,
		ARRAY_SIZE(m7evm_slim_devices));
	m7evm_init_dsps();
...
static void __init m7evm_allocate_memory_regions(void)
{
	m7china_allocate_fb_region();
}

static void __init m7evm_cdp_init(void)
{
...
static void __init m7evm_fixup(struct tag *tags, char **cmdline, struct meminfo *mi)
{
...
MACHINE_START(M7_EVM, "UNKNOWN")
	.fixup = m7evm_fixup,
	.map_io = m7evm_map_io,
	.reserve = m7evm_reserve,
	.init_irq = m7evm_init_irq,
	.handle_irq = gic_handle_irq,
	.timer = &msm_timer,
	.init_machine = m7evm_cdp_init,
	.init_early = m7evm_allocate_memory_regions,
	.init_very_early = m7evm_early_reserve,
	.restart = msm_restart,
MACHINE_END
