/* linux/arch/arm/mach-msm/board-zara_cl-mmc.c
 *
...
static uint32_t wifi_on_gpio_table[] = {
	...
	GPIO_CFG(MSM_WCN_CMD_CLK_CPU, 2, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA), /* CLK */
...
static uint32_t wifi_off_gpio_table[] = {
	...
	GPIO_CFG(MSM_WCN_CMD_CLK_CPU, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA), /* CLK */
 ...
 */
static struct embedded_sdio_data zara_cl_wifi_emb_data = {
	.cccr	= {
		.sdio_vsn	= 2,
		.multi_block	= 1,
		.low_speed	= 0,
...
static int
zara_cl_wifi_status_register(void (*callback)(int card_present, void *dev_id),
				void *dev_id)
{
	if (wifi_status_cb)
		return -EAGAIN;
...
static int zara_cl_wifi_cd;	/* WiFi virtual 'card detect' status */

static unsigned int zara_cl_wifi_status(struct device *dev)
{
	return zara_cl_wifi_cd;
}

static unsigned int zara_cl_wifislot_type = MMC_TYPE_SDIO_WIFI;
static unsigned int wifi_sup_clk_rates[] = {
	400000, 24000000, 48000000,
};
static struct mmc_platform_data zara_cl_wifi_data = {
	.ocr_mask               = MMC_VDD_28_29,
	.status                 = zara_cl_wifi_status,
	.register_status_notify = zara_cl_wifi_status_register,
	.embedded_sdio          = &zara_cl_wifi_emb_data,
	.mmc_bus_width  = MMC_CAP_4_BIT_DATA,
	.sup_clk_table	= wifi_sup_clk_rates,
	.sup_clk_cnt	= ARRAY_SIZE(wifi_sup_clk_rates),
	.slot_type = &zara_cl_wifislot_type,
...
int zara_cl_wifi_set_carddetect(int val)
{
...
EXPORT_SYMBOL(zara_cl_wifi_set_carddetect);
...
EXPORT_SYMBOL(zara_cl_wifi_power);
...
/* ---- MMC ---- */
int __init zara_cl_init_mmc()
{
	uint32_t id;
	struct pm_gpio pm_config;
	wifi_status_cb = NULL;

	printk(KERN_INFO "zara_cl: %s\n", __func__);
	...
	/* PM QoS for wifi*/
	zara_cl_wifi_data.cpu_dma_latency = msm_rpmrs_levels[0].latency_us;

	msm_add_sdcc(4, &zara_cl_wifi_data);
