...
{
	pr_info("get_hw_revisio:skuid_audio=0x%x, system_rev=%x\n", skuid_audio, system_rev);
	if ((skuid_audio & 0xFFF00) == 0x36B00)
	{
		pr_info("return 0 for STMIC");
		return 0;
	}
	else if ((skuid_audio & 0xFFF00) == 0x3B300)
	{
		pr_info("return 1 for KMIC");
		return 1;
	}
	else
	{
		pr_info("SKUID is not defined, return 0 for STMIC");
		return 0;
	}
}
