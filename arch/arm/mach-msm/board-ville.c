...
/*
int ville_mdp_color_enhance(void)
{
	mdp_color_enhancement(ville_sony_nt_color_v01, ARRAY_SIZE(ville_sony_nt_color_v01));
	return 0;
}
*/
...
#define HBOOT_RAMDUMP_END 0x88000000
				 ...
				 char **cmdline, struct meminfo *mi)
{
	...
	// hboot dump 0x80000000 ~ 0x880000000
	if (mem_size_mb == 64) {
		printk(KERN_INFO "%s: setup memory(%uMB) for minikernel dump\n", __func__, mem_size_mb);
		mi->nr_banks = 1;
		mi->bank[0].start = PHY_BASE_ADDR1;
		mi->bank[0].size = HBOOT_RAMDUMP_END - PHY_BASE_ADDR1;
