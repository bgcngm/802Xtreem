/* linux/arch/arm/mach-msm/board-k2_ul-wifi.c
*/
...
#include "board-k2_ul-wifi.h"

int k2_ul_wifi_power(int on);
int k2_ul_wifi_reset(int on);
int k2_ul_wifi_set_carddetect(int on);
int k2_ul_wifi_get_mac_addr(unsigned char *buf);
...
static void *k2_ul_wifi_mem_prealloc(int section, unsigned long size)
{
...
int __init k2_ul_init_wifi_mem(void)
{
	int i;
...
static struct resource k2_ul_wifi_resources[] = {
	[0] = {
...
static struct platform_device k2_ul_wifi_device = {
	.name           = "bcmdhd_wlan",
	.id             = 1,
	.num_resources  = ARRAY_SIZE(k2_ul_wifi_resources),
	.resource       = k2_ul_wifi_resources,
	.dev            = {
		.platform_data = &k2_ul_wifi_control,
	},
};

static unsigned k2_ul_wifi_update_nvs(char *str)
{
...
#else
	k2_ul_wifi_update_nvs("sd_oobonly=1\n");
#endif
	k2_ul_wifi_update_nvs("btc_params80=0\n");
	k2_ul_wifi_update_nvs("btc_params6=30\n");
	k2_ul_init_wifi_mem();
	ret = platform_device_register(&k2_ul_wifi_device);
	return ret;
}
