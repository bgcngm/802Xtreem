...
static int dlp_china_detect_panel(const char *name)
{
#if 0
...
static struct msm_fb_platform_data msm_fb_pdata = {
	.detect_client = dlp_china_detect_panel,
};
...
void __init dlp_china_allocate_fb_region(void)
{
	void *addr;
	unsigned long size;
...
void __init dlp_china_mdp_writeback(struct memtype_reserve* reserve_table)
{
...
static struct mipi_dsi_panel_platform_data *mipi_dlp_china_pdata;

static struct dsi_buf dlp_china_panel_tx_buf;
static struct dsi_buf dlp_china_panel_rx_buf;
...
static int dlp_china_lcd_on(struct platform_device *pdev)
{
	...
	if(!first_init) {
		if (mipi->mode == DSI_VIDEO_MODE) {
			mipi_dsi_cmds_tx(&dlp_china_panel_tx_buf, video_on_cmds,
				video_on_cmds_count);
		}
	}
	first_init = 0;
...
static int dlp_china_lcd_off(struct platform_device *pdev)
{
	struct msm_fb_data_type *mfd;
...
static int __devinit dlp_china_lcd_probe(struct platform_device *pdev)
{
	if (pdev->id == 0) {
		mipi_dlp_china_pdata = pdev->dev.platform_data;
		return 0;
	}
...
static void dlp_china_display_on(struct msm_fb_data_type *mfd)
{
...
static void dlp_china_display_off(struct msm_fb_data_type *mfd)
{
...
static unsigned char dlp_china_shrink_pwm(int val)
{
...
static struct platform_driver this_driver = {
	.probe  = dlp_china_lcd_probe,
	.driver = {
		.name   = "mipi_dlp_china",
	},
};

static struct msm_fb_panel_data dlp_china_panel_data = {
	.on	= dlp_china_lcd_on,
	.off	= dlp_china_lcd_off,
	.set_backlight = dlp_china_set_backlight,
	.display_on = dlp_china_display_on,
	.display_off = dlp_china_display_off,
};

static struct msm_panel_info pinfo;
static int ch_used[3] = {0};

static int mipi_dlp_china_device_register(struct msm_panel_info *pinfo,
					u32 channel, u32 panel)
{
	...
	pdev = platform_device_alloc("mipi_dlp_china", (panel << 8)|channel);
	if (!pdev)
		return -ENOMEM;

	dlp_china_panel_data.panel_info = *pinfo;

	ret = platform_device_add_data(pdev, &dlp_china_panel_data,
		sizeof(dlp_china_panel_data));
	if (ret) {
...
static int __init mipi_video_sharp_init(void)
{
	...
	ret = mipi_dlp_china_device_register(&pinfo, MIPI_DSI_PRIM,
...
void __init dlp_china_init_fb(void)
{
...
static int __init dlp_china_panel_init(void)
{
	int ret;
	...
	mipi_dsi_buf_alloc(&dlp_china_panel_tx_buf, DSI_BUF_SIZE);
	mipi_dsi_buf_alloc(&dlp_china_panel_rx_buf, DSI_BUF_SIZE);
...
device_initcall_sync(dlp_china_panel_init);
