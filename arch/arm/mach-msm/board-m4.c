...
/* Adjust driving strength for PVT on which UART switch is removed */
	...
	if ( machine_is_m4_ul() )
		android_usb_pdata.product_id = 0x0dcd;/* MFR#UL */
	else if ( machine_is_m4_u())
		android_usb_pdata.product_id = 0x0dce;/* MFR#U */
...
static struct synaptics_i2c_rmi_platform_data syn_ts_3k_data[] = {
	{
		...
		.config = {
			...
			0x4D, 0x46, 0x31, 0x30, 0x00, 0x7F, 0x03, 0x1E,
...
static struct mpu3050_platform_data mpu3050_data = {
	...
	.accel = {
		...
		.adapt_num = MSM_8930_GSBI12_QUP_I2C_BUS_ID, /* The i2c bus to which the mpu device is connected */
				 ...
				 char **cmdline, struct meminfo *mi)
{
	...
	skuid_audio = parse_tag_skuid((const struct tag *)tags);
	printk(KERN_INFO "m4_fixup:skuid=0x%x\n", skuid);
	printk(KERN_INFO "m4_fixup:skuid_audio=0x%x\n", skuid_audio);
}
