...
static void zip_cl_amp_speaker(bool en)
{
...
static void zip_cl_amp_hac(bool en)
{
}

struct htc_8930_gpio_pdata htc_audio_gpio = {
	.amp_speaker = zip_cl_amp_speaker,
	.amp_receiver = zip_cl_amp_receiver,
	.amp_headset = zip_cl_amp_headset,
	.amp_hac = zip_cl_amp_hac,
	.mi2s_gpio = {
	{
...
static int zip_cl_get_hw_revision(void)
{
	int audio_hw_rev;
...
static int zip_cl_get_hw_component(void)
{
	int hw_com = 0;
...
static int zip_cl_enable_digital_mic(void)
{
	return 1;
}

static void zip_cl_set_q6_effect_mode(int mode)
{
	pr_info("%s: mode %d\n", __func__, mode);
	atomic_set(&q6_effect_mode, mode);
}

static int zip_cl_get_q6_effect_mode(void)
{
...
static int zip_cl_get_24b_audio(void)
{
	return 1;
}

static struct acoustic_ops acoustic = {
	.set_q6_effect = zip_cl_set_q6_effect_mode,
	.get_htc_revision = zip_cl_get_hw_revision,
	.get_hw_component = zip_cl_get_hw_component,
	.enable_digital_mic = zip_cl_enable_digital_mic,
	.get_24b_audio = zip_cl_get_24b_audio,
};

static struct q6asm_ops qops = {
	.get_q6_effect = zip_cl_get_q6_effect_mode,
};
...
static int __init zip_cl_audio_init(void)
{
	int ret = 0;
	pr_info("%s", __func__);
...
module_init(zip_cl_audio_init);

static void __exit zip_cl_audio_exit(void)
{
	pr_info("%s", __func__);
}
module_exit(zip_cl_audio_exit);

MODULE_DESCRIPTION("ALSA Board ZIPCL");
