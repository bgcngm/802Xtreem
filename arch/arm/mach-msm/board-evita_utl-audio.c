 ...
 *
 * HTC: evita_utl machine driver which defines board-specific data
...
#define EVITA_UTL_AUD_STEREO_REC	(PM8921_GPIO_PM_TO_SYS(3))
...
static const struct snd_soc_dapm_widget evita_utl_dapm_widgets[] = {
...
static int msm8960_audrx_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	snd_soc_dapm_new_controls(dapm, evita_utl_dapm_widgets,
				ARRAY_SIZE(evita_utl_dapm_widgets));
...
void evita_utl_set_q6_effect_mode(int mode)
{
	pr_aud_info("%s: mode %d\n", __func__, mode);
	atomic_set(&q6_effect_mode, mode);
}

int evita_utl_get_q6_effect_mode(void)
{
...
int evita_utl_get_component_info(void)
{
	int ret = 0;
	uint32_t audio_hw_comp = HTC_AUDIO_A1028;

	ret = gpio_request(EVITA_UTL_GPIO_NC_97, "audience_detect");
	if (ret) {
		pr_err("%s: Error requesting Audience Detection GPIO %u\n",
			__func__, EVITA_UTL_GPIO_NC_97);
	 ...
	 */
	if (gpio_get_value(EVITA_UTL_GPIO_NC_97)) {
		audio_hw_comp &= (~HTC_AUDIO_A1028);
		pr_info("%s, HW without Audience 1028\n", __func__);
	}

	gpio_free(EVITA_UTL_GPIO_NC_97);
	return audio_hw_comp;
}

static struct acoustic_ops acoustic = {
	.set_q6_effect = evita_utl_set_q6_effect_mode,
	 .get_hw_component = evita_utl_get_component_info,
};

static struct q6asm_ops qops = {
	.get_q6_effect = evita_utl_get_q6_effect_mode,
};
...
static int __init evita_utl_audio_init(void)
{
	int ret;
...
late_initcall(evita_utl_audio_init);

static void __exit evita_utl_audio_exit(void)
{
...
module_exit(evita_utl_audio_exit);

MODULE_DESCRIPTION("ALSA Platform Evita_UTL");
