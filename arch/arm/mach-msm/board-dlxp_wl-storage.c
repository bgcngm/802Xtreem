...
uint32_t sdc4_gpio_on[] = {
	GPIO_CFG(63, 2, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_6MA),
								//"sdc4_dat_3"
	GPIO_CFG(64, 2, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_6MA),
								//"sdc4_dat_2"
	GPIO_CFG(65, 2, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_6MA),
								//"sdc4_dat_1"
	GPIO_CFG(66, 2, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_6MA),
								//"sdc4_dat_0"
	GPIO_CFG(67, 2, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_6MA),
								//"sdc4_cmd"
	GPIO_CFG(68, 2, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
								//"sdc4_clk"
};
...
static struct mmc_platform_data *dlxp_wl_sdc1_pdata = &sdc1_data;
#else
static struct mmc_platform_data *dlxp_wl_sdc1_pdata;
#endif
 ...
 */
static struct embedded_sdio_data dlxp_wl_wifi_emb_data = {
	.cccr	= {
		.sdio_vsn	= 2,
		.multi_block	= 1,
		.low_speed	= 0,
...
static int
dlxp_wl_wifi_status_register(void (*callback)(int card_present, void *dev_id),
				void *dev_id)
{
	if (wifi_status_cb)
		return -EAGAIN;
...
static int dlxp_wl_wifi_cd;	/* WiFi virtual 'card detect' status */

static unsigned int dlxp_wl_wifi_status(struct device *dev)
{
	return dlxp_wl_wifi_cd;
}

static unsigned int dlxp_wl_wifislot_type = MMC_TYPE_SDIO_WIFI;
static unsigned int wifi_sup_clk_rates[] = {
	400000, 24000000, 48000000, 96000000
};
static struct mmc_platform_data dlxp_wl_wifi_data = {
	.ocr_mask               = MMC_VDD_28_29,
	.status                 = dlxp_wl_wifi_status,
	.register_status_notify = dlxp_wl_wifi_status_register,
	.embedded_sdio          = &dlxp_wl_wifi_emb_data,
	.mmc_bus_width  = MMC_CAP_4_BIT_DATA,
	.slot_type = &dlxp_wl_wifislot_type,
...
int dlxp_wl_wifi_set_carddetect(int val)
{
...
EXPORT_SYMBOL(dlxp_wl_wifi_set_carddetect);
...
EXPORT_SYMBOL(dlxp_wl_wifi_power);

int dlxp_wl_wifi_reset(int on)
{
...
void __init dlxp_wl_init_mmc(void)
{
	wifi_status_cb = NULL;

	printk(KERN_INFO "dlxp_wl: %s\n", __func__);
...
#if 0
	/* PM QoS for wifi*/
    dlxp_wl_wifi_data.swfi_latency = msm_rpm_get_swfi_latency();
#endif

	apq8064_add_sdcc(1, dlxp_wl_sdc1_pdata);
	apq8064_add_sdcc(3, &dlxp_wl_wifi_data);
