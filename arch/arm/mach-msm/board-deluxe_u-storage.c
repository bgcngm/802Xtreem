...
static struct mmc_platform_data *deluxe_u_sdc1_pdata = &sdc1_data;
#else
static struct mmc_platform_data *deluxe_u_sdc1_pdata;
#endif
...
static struct mmc_platform_data *deluxe_u_sdc3_pdata = &sdc3_data;
#else
static struct mmc_platform_data *deluxe_u_sdc3_pdata;
#endif
#endif
 ...
 */
static struct embedded_sdio_data deluxe_u_wifi_emb_data = {
	.cccr	= {
		.sdio_vsn	= 2,
		.multi_block	= 1,
		.low_speed	= 0,
...
static int
deluxe_u_wifi_status_register(void (*callback)(int card_present, void *dev_id),
				void *dev_id)
{
	if (wifi_status_cb)
		return -EAGAIN;
...
static int deluxe_u_wifi_cd;	/* WiFi virtual 'card detect' status */

static unsigned int deluxe_u_wifi_status(struct device *dev)
{
	return deluxe_u_wifi_cd;
}

static unsigned int deluxe_u_wifislot_type = MMC_TYPE_SDIO_WIFI;
static unsigned int wifi_sup_clk_rates[] = {
	400000, 24000000, 48000000
};
static struct mmc_platform_data deluxe_u_wifi_data = {
	.ocr_mask               = MMC_VDD_28_29,
	.status                 = deluxe_u_wifi_status,
	.register_status_notify = deluxe_u_wifi_status_register,
	.embedded_sdio          = &deluxe_u_wifi_emb_data,
	.mmc_bus_width  = MMC_CAP_4_BIT_DATA,
	.slot_type = &deluxe_u_wifislot_type,
...
int deluxe_u_wifi_set_carddetect(int val)
{
	printk(KERN_INFO "%s: %d\n", __func__, val);
	deluxe_u_wifi_cd = val;
	if (wifi_status_cb)
...
EXPORT_SYMBOL(deluxe_u_wifi_set_carddetect);
...
EXPORT_SYMBOL(deluxe_u_wifi_power);
