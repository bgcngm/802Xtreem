...
static int dlxp_wl_get_hw_component(void)
{
    int hw_com = 0;
...
static int dlxp_wl_enable_digital_mic(void)
{
    printk(KERN_INFO "dlxp_wl_enable_digital_mic:skuid=0x%x, system_rev=%x\n", skuid, system_rev);
    return 1;
}
...
static struct acoustic_ops acoustic = {
        .enable_digital_mic = dlxp_wl_enable_digital_mic,
        .get_hw_component = dlxp_wl_get_hw_component,
...
static int __init dlxp_wl_audio_init(void)
{
        int ret = 0;
...
late_initcall(dlxp_wl_audio_init);

static void __exit dlxp_wl_audio_exit(void)
{
	pr_info("%s", __func__);
}
module_exit(dlxp_wl_audio_exit);
