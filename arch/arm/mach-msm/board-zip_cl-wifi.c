/* linux/arch/arm/mach-msm/board-zip_cl-wifi.c
*/
...
#include "board-zip_cl-wifi.h"

int zip_cl_wifi_power(int on);
int zip_cl_wifi_reset(int on);
int zip_cl_wifi_set_carddetect(int on);
int zip_cl_wifi_get_mac_addr(unsigned char *buf);
...
static void *zip_cl_wifi_mem_prealloc(int section, unsigned long size)
{
...
int __init zip_cl_init_wifi_mem(void)
{
	int i;
...
static struct resource zip_cl_wifi_resources[] = {
	[0] = {
...
static struct wifi_platform_data zip_cl_wifi_control = {
	.set_power      = zip_cl_wifi_power,
	.set_reset      = zip_cl_wifi_reset,
	.set_carddetect = zip_cl_wifi_set_carddetect,
	.mem_prealloc   = zip_cl_wifi_mem_prealloc,
	.get_mac_addr	= zip_cl_wifi_get_mac_addr,
	.bus_scale_table    = &wlan_bus_scale_pdata,
};

static struct platform_device zip_cl_wifi_device = {
	.name           = "bcmdhd_wlan",
	.id             = 1,
	.num_resources  = ARRAY_SIZE(zip_cl_wifi_resources),
	.resource       = zip_cl_wifi_resources,
	.dev            = {
		.platform_data = &zip_cl_wifi_control,
	},
};

static unsigned zip_cl_wifi_update_nvs(char *str)
{
...
#else
	zip_cl_wifi_update_nvs("sd_oobonly=1\n");
#endif
	zip_cl_wifi_update_nvs("btc_params80=0\n");
	zip_cl_wifi_update_nvs("btc_params6=30\n");
	zip_cl_init_wifi_mem();
	ret = platform_device_register(&zip_cl_wifi_device);
	return ret;
}
