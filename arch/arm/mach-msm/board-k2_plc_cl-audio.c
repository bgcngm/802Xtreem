...
static int k2plccl_get_hw_revision(void)
{
	int audio_hw_rev;
...
static int k2plccl_get_hw_component(void)
{
	return 0;
}

static int k2plccl_enable_digital_mic(void)
{
	return 0;
}

static void k2plccl_set_q6_effect_mode(int mode)
{
	pr_info("%s: mode %d\n", __func__, mode);
	atomic_set(&q6_effect_mode, mode);
}

static int k2plccl_get_q6_effect_mode(void)
{
...
static int k2plccl_get_24b_audio(void)
{
	return 1;
}
...
static struct acoustic_ops acoustic = {
	...
	.get_hw_component = k2plccl_get_hw_component,
	.enable_digital_mic = k2plccl_enable_digital_mic,
	.get_24b_audio = k2plccl_get_24b_audio,
};
...
static int __init k2plccl_audio_init(void)
{
	int ret = 0;
	pr_info("%s", __func__);
...
module_init(k2plccl_audio_init);

static void __exit k2plccl_audio_exit(void)
{
	pr_info("%s", __func__);
}
module_exit(k2plccl_audio_exit);

MODULE_DESCRIPTION("ALSA Board K2PLCCL");
