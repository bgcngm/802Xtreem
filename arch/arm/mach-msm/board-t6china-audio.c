...
static int t6china_enable_digital_mic(void)
{
	...
	printk(KERN_INFO "t6china_enable_digital_mic[ret: %d]:skuid=0x%x, system_rev=%x\n", ret, skuid, system_rev);
	return ret;
}
...
static int __init t6china_audio_init(void)
{
        int ret = 0;
...
late_initcall(t6china_audio_init);

static void __exit t6china_audio_exit(void)
{
	pr_info("%s", __func__);
}
module_exit(t6china_audio_exit);
