...
static int evita_utl_csi_vreg_on(void);
static int evita_utl_csi_vreg_off(void);
...
struct msm_camera_device_platform_data msm_camera_csi_device_data[] = {
	{
		...
		.csid_core = 0,
		.camera_csi_on = evita_utl_csi_vreg_on,
		.camera_csi_off = evita_utl_csi_vreg_off,
...
static int evita_utl_use_ext_1v2(void)
{
		return 1;
}

#define EVITA_UTL_V_RAW_1V2_EN PM8921_GPIO_PM_TO_SYS(EVITA_UTL_PMGPIO_V_RAW_1V2_EN)
static int evita_utl_rawchip_vreg_on(void)
{
	int rc;
	pr_info("%s\n", __func__);

	/* PM8921_GPIO_PM_TO_SYS(EVITA_UTL_GPIO_V_RAW_1V8_EN) 1800000 */
	rc = gpio_request(EVITA_UTL_GPIO_V_RAW_1V8_EN, "V_RAW_1V8_EN");
	if (rc) {
		pr_err("rawchip on\
			(\"gpio %d\", 1.2V) FAILED %d\n",
			EVITA_UTL_GPIO_V_RAW_1V8_EN, rc);
		goto enable_1v8_fail;
	}
	gpio_direction_output(EVITA_UTL_GPIO_V_RAW_1V8_EN, 1);
	gpio_free(EVITA_UTL_GPIO_V_RAW_1V8_EN);

	mdelay(5);

	/* PM8921_GPIO_PM_TO_SYS(EVITA_UTL_V_RAW_1V2_EN) 1200000 */
	rc = gpio_request(EVITA_UTL_V_RAW_1V2_EN, "V_RAW_1V2_EN");
	if (rc) {
		pr_err("rawchip on\
			(\"gpio %d\", 1.2V) FAILED %d\n",
			EVITA_UTL_V_RAW_1V2_EN, rc);
		goto enable_1v2_fail;
	}
	gpio_direction_output(EVITA_UTL_V_RAW_1V2_EN, 1);
	gpio_free(EVITA_UTL_V_RAW_1V2_EN);

	if (evita_utl_use_ext_1v2()) { /* use external 1v2 for HW workaround */
		mdelay(1);

		rc = gpio_request(EVITA_UTL_GPIO_V_CAM_D1V2_EN, "rawchip");
		pr_info("rawchip external 1v2 gpio_request,%d\n", EVITA_UTL_GPIO_V_CAM_D1V2_EN);
		if (rc < 0) {
			pr_err("GPIO(%d) request failed", EVITA_UTL_GPIO_V_CAM_D1V2_EN);
			goto enable_ext_1v2_fail;
		}
		gpio_direction_output(EVITA_UTL_GPIO_V_CAM_D1V2_EN, 1);
		gpio_free(EVITA_UTL_GPIO_V_CAM_D1V2_EN);
	}
...
enable_ext_1v2_fail:
	...
	if (rc)
		pr_err("rawchip on\
			(\"gpio %d\", 1.2V) FAILED %d\n",
			EVITA_UTL_V_RAW_1V2_EN, rc);
	gpio_direction_output(EVITA_UTL_V_RAW_1V2_EN, 0);
	gpio_free(EVITA_UTL_V_RAW_1V2_EN);
enable_1v2_fail:
	rc = gpio_request(EVITA_UTL_GPIO_V_RAW_1V8_EN, "V_RAW_1V8_EN");
	if (rc)
		pr_err("rawchip on\
			(\"gpio %d\", 1.2V) FAILED %d\n",
			EVITA_UTL_GPIO_V_RAW_1V8_EN, rc);
	gpio_direction_output(EVITA_UTL_GPIO_V_RAW_1V8_EN, 0);
	gpio_free(EVITA_UTL_GPIO_V_RAW_1V8_EN);
enable_1v8_fail:
	return rc;
}

static int evita_utl_rawchip_vreg_off(void)
{
	int rc = 0;
	...
	if (evita_utl_use_ext_1v2()) { /* use external 1v2 for HW workaround */
		...
		if (rc)
			pr_err("rawchip off(\
				\"gpio %d\", 1.2V) FAILED %d\n",
				EVITA_UTL_GPIO_V_CAM_D1V2_EN, rc);
		gpio_direction_output(EVITA_UTL_GPIO_V_CAM_D1V2_EN, 0);
		gpio_free(EVITA_UTL_GPIO_V_CAM_D1V2_EN);
		mdelay(1);
	}
...
static struct msm_camera_rawchip_info msm_rawchip_board_info = {
	.rawchip_reset	= EVITA_UTL_GPIO_RAW_RSTN,
	.rawchip_intr0	= MSM_GPIO_TO_INT(EVITA_UTL_GPIO_RAW_INTR0),
	.rawchip_intr1	= MSM_GPIO_TO_INT(EVITA_UTL_GPIO_RAW_INTR1),
	...
	.camera_rawchip_power_on = evita_utl_rawchip_vreg_on,
	.camera_rawchip_power_off = evita_utl_rawchip_vreg_off,
	.rawchip_use_ext_1v2 = evita_utl_use_ext_1v2,
};
...
static uint16_t msm_cam_gpio_tbl[] = {
	EVITA_UTL_GPIO_CAM_MCLK0, /*CAMIF_MCLK*/
	EVITA_UTL_GPIO_CAM_MCLK1,
	EVITA_UTL_GPIO_RAW_INTR0,
	EVITA_UTL_GPIO_RAW_INTR1,
	EVITA_UTL_GPIO_MCAM_SPI_CLK,
	EVITA_UTL_GPIO_MCAM_SPI_CS0,
	EVITA_UTL_GPIO_MCAM_SPI_DI,
	EVITA_UTL_GPIO_MCAM_SPI_DO,
};
...
static int evita_utl_s5k3h2yx_vreg_on(void)
{
	int rc;
	pr_info("%s\n", __func__);
...
static int evita_utl_s5k3h2yx_vreg_off(void)
{
	int rc = 0;
...
static struct msm_actuator_info s5k3h2yx_actuator_info = {
	...
	.vcm_pwd        = EVITA_UTL_GPIO_CAM_VCM_PD,
	.vcm_enable     = 1,
};
#endif
...
static struct msm_camera_sensor_platform_info sensor_s5k3h2yx_board_info = {
	...
	.sensor_reset	= 0,
	.sensor_pwd	= EVITA_UTL_GPIO_CAM_PWDN,
	.vcm_pwd	= EVITA_UTL_GPIO_CAM_VCM_PD,
	.vcm_enable	= 1,
...
static struct msm_camera_sensor_info msm_camera_sensor_s5k3h2yx_data = {
	...
	.camera_power_on = evita_utl_s5k3h2yx_vreg_on,
	.camera_power_off = evita_utl_s5k3h2yx_vreg_off,
...
static int evita_utl_imx175_vreg_on(void)
{
	int rc;
	pr_info("[CAM] %s\n", __func__);
...
static int evita_utl_imx175_vreg_off(void)
{
	int rc = 0;
...
static struct msm_camera_sensor_info msm_camera_sensor_imx175_data = {
	.sensor_name	= "imx175",
	.camera_power_on = evita_utl_imx175_vreg_on,
	.camera_power_off = evita_utl_imx175_vreg_off,
	.pdata	= &msm_camera_csi_device_data[0],
...
#define EVITA_UTL_XB_GPIO_V_CAM2_D1V2_EN 93
static int evita_utl_s5k6a1gx_vreg_on(void)
{
	int rc;
	pr_info("%s\n", __func__);
	...
	/* reset pin */
	rc = gpio_request(EVITA_UTL_GPIO_CAM2_RSTz, "s5k6a1gx");
	pr_info("reset pin gpio_request,%d\n", EVITA_UTL_GPIO_CAM2_RSTz);
	if (rc < 0) {
		pr_err("GPIO(%d) request failed", EVITA_UTL_GPIO_CAM2_RSTz);
		goto enable_rst_fail;
	}
	gpio_direction_output(EVITA_UTL_GPIO_CAM2_RSTz, 1);
	gpio_free(EVITA_UTL_GPIO_CAM2_RSTz);
	udelay(50);

	/* digital */
	rc = gpio_request(EVITA_UTL_XB_GPIO_V_CAM2_D1V2_EN, "s5k6a1gx");
	pr_info("reset pin gpio_request,%d\n", EVITA_UTL_XB_GPIO_V_CAM2_D1V2_EN);
	if (rc < 0) {
		pr_err("GPIO(%d) request failed", EVITA_UTL_XB_GPIO_V_CAM2_D1V2_EN);
		goto enable_digital_fail;
	}
	gpio_direction_output(EVITA_UTL_XB_GPIO_V_CAM2_D1V2_EN, 1);
	gpio_free(EVITA_UTL_XB_GPIO_V_CAM2_D1V2_EN);
	udelay(50);
...
enable_digital_fail:
	...
	if (rc < 0)
		...
		gpio_direction_output(EVITA_UTL_GPIO_CAM2_RSTz, 0);
		gpio_free(EVITA_UTL_GPIO_CAM2_RSTz);
	}
enable_rst_fail:
...
static int evita_utl_s5k6a1gx_vreg_off(void)
{
	int rc;
	pr_info("%s\n", __func__);
	...
	if (rc < 0)
		...
		gpio_direction_output(EVITA_UTL_XB_GPIO_V_CAM2_D1V2_EN, 0);
		gpio_free(EVITA_UTL_XB_GPIO_V_CAM2_D1V2_EN);
	}
	udelay(50);
...
static struct msm_camera_sensor_platform_info sensor_s5k6a1gx_board_info = {
	...
	.mirror_flip = CAMERA_SENSOR_NONE,
	.sensor_reset_enable = 0,
	.sensor_reset	= EVITA_UTL_GPIO_CAM2_RSTz,
	.sensor_pwd	= EVITA_UTL_GPIO_CAM2_STANDBY,
	.vcm_pwd	= 0,
	.vcm_enable	= 0,
...
static struct msm_camera_sensor_info msm_camera_sensor_s5k6a1gx_data = {
	...
	.vcm_pwd	= 0,
	.vcm_enable	= 0,
	.camera_power_on = evita_utl_s5k6a1gx_vreg_on,
	.camera_power_off = evita_utl_s5k6a1gx_vreg_off,
...
static void config_cam_id(int status)
{
	static uint32_t cam_id_gpio_start[] = {
		GPIO_CFG(EVITA_UTL_GPIO_MAIN_CAM_ID, 1, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA),
	};

	static uint32_t cam_id_gpio_end[] = {
		GPIO_CFG(EVITA_UTL_GPIO_MAIN_CAM_ID, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
	};
...
struct msm_camera_board_info evita_utl_camera_board_info = {
...
struct msm_camera_board_info evita_utl_camera_board_info_2nd = {
...
void __init msm8960_init_cam(void)
{
	...
	if (gpio_get_value(EVITA_UTL_GPIO_MAIN_CAM_ID) == 0){
		i2c_register_board_info(MSM_8960_GSBI4_QUP_I2C_BUS_ID,
			evita_utl_camera_board_info.board_info,
			evita_utl_camera_board_info.num_i2c_board_info);
	}else{ /* for 2nd Source */
		i2c_register_board_info(MSM_8960_GSBI4_QUP_I2C_BUS_ID,
			evita_utl_camera_board_info_2nd.board_info,
			evita_utl_camera_board_info_2nd.num_i2c_board_info);
	}
