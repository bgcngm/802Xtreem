...
#define MSM_ION_MM_RELAYOUT 1 /* move all zip ion heaps to Memory Pool */
#ifdef CONFIG_MSM_MULTIMEDIA_USE_ION
#define HOLE_SIZE	0x20000
#define MSM_PMEM_KERNEL_EBI1_SIZE  0x65000
#ifdef CONFIG_MSM_IOMMU
#define MSM_ION_MM_SIZE            0x4700000 /* Need to be multiple of 64K */
...
static void reserve_ion_memory(void)
{
	...
	msm8930_reserve_table[MEMTYPE_EBI1].size += MSM_ION_ROTATOR_SIZE;
#endif
#endif
}
#else
...
{
	zip_cl_mdp_writeback(msm8930_reserve_table);
}
...
static void __init zip_cl_early_memory(void)
{
...
static void __init zip_cl_reserve(void)
{
	msm_reserve();
	if (msm8930_fmem_pdata.size) {
#if defined(CONFIG_ION_MSM) && defined(CONFIG_MSM_MULTIMEDIA_USE_ION)
...
int64_t k2_zip_cl_get_usbid_adc(void)
{
...
void config_k2_zip_cl_usb_id_gpios(bool output)
{
	if (output) {
...
static struct cable_detect_platform_data cable_detect_pdata = {
	...
	.get_adc_cb		= k2_zip_cl_get_usbid_adc,
	.config_usb_id_gpios	= config_k2_zip_cl_usb_id_gpios,
};
...
void zip_cl_cable_detect_register(void)
{
...
static void __init zip_cl_map_io(void)
{
...
static void __init zip_cl_init_irq(void)
{
...
static struct cm3629_platform_data cm36282_pdata = {
	...
	.levels = { 14, 48, 72, 322, 753, 3829, 6091, 11649, 17208, 65535},
	.golden_adc = 0x1104,
	.power = cm3629_power,
...
static int zip_cl_phy_init_seq[] = {
	0x3C, 0x81,
	0x24, 0x82,
	-1
};

static int zip_cl_phy_init_seq_no_uart[] = {
	0x3C, 0x81,
	0x24, 0x82,
	-1
};
...
static struct msm_otg_platform_data msm_otg_pdata = {
	.phy_init_seq	= zip_cl_phy_init_seq,
	.mode			= USB_OTG,
...
#define ZIP_CL_GPIO_CAM_I2C_DAT 	MSM_CAM_I2C_SDA
#define ZIP_CL_GPIO_CAM_I2C_CLK 	MSM_CAM_I2C_SCL
...
static uint32_t gsbi4_gpio_table[] = {
	GPIO_CFG(ZIP_CL_GPIO_CAM_I2C_DAT, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
	GPIO_CFG(ZIP_CL_GPIO_CAM_I2C_CLK, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA),
};

static uint32_t gsbi4_gpio_table_gpio[] = {
	GPIO_CFG(ZIP_CL_GPIO_CAM_I2C_DAT, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA),
	GPIO_CFG(ZIP_CL_GPIO_CAM_I2C_CLK, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA),
};
...
static struct synaptics_virtual_key zip_vk[] = {
	{
		.index = 1,
		.keycode = KEY_BACK,
		.x_range_min = 180,
...
static struct synaptics_i2c_rmi_platform_data syn_ts_3k_data[] = {
	{
		...
		.support_htc_event = 1,
		.config = {
			,
			...
			0x00, 0x02, 0x36, 0x3C, 0x08, 0x84, 0x04, 0x20,
			0x02,
		},
	},
	{
	...
	},
	{
		...
		.support_htc_event = 1,
		.config = {
			 0x5A, 0x50, 0x30, 0x32, 0x00, 0x7F, 0x03, 0x1E,
			 ...
			 0x01, 0x80, 0x03, 0x0E, 0x1F, 0x11, 0x43, 0x00,
			 0x13, 0x04, 0x1B, 0x00, 0x10, 0x0A, 0x80, 0x60,
			 0x60, 0x68, 0x68, 0x48, 0x48, 0x48, 0x35, 0x33,
			 0x30, 0x2D, 0x2A, 0x27, 0x24, 0x22, 0x00, 0x00,
			 0x00, 0x03, 0x08, 0x0D, 0x14, 0x1B, 0x00, 0x88,
			 0x13, 0x00, 0x64, 0x00, 0xC8, 0x00, 0x80, 0x14,
			 0xB3, 0x88, 0x13, 0x00, 0xC0, 0x19, 0x02, 0x02,
			 ...
			 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
			 0x00, 0x00, 0x00, 0x03, 0x00, 0x12, 0x06, 0x0D,
			 0x07, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
			 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x9A, 0x9A, 0x51,
			 0x51, 0x51, 0x51, 0x51, 0x51, 0xCD, 0x0D, 0x04,
			 0x00, 0x08, 0x0A, 0x09, 0x0E, 0x0F, 0x12, 0x14,
			 0x00, 0x01, 0x02, 0x03, 0x04, 0x0C, 0x0D, 0x0B,
			 0x15, 0x17, 0x16, 0x18, 0x19, 0x1A, 0x1B, 0x05,
			 0x06, 0x07, 0xFF, 0xFF, 0x0B, 0x0A, 0x04, 0x05,
			 0x02, 0x06, 0x01, 0x0C, 0x07, 0x08, 0x0E, 0x0F,
			 0x10, 0x12, 0x0D, 0xFF, 0x00, 0x10, 0x00, 0x10,
...
static void syn_init_vkeys_zip_cl(void)
{
	int rc = 0;
...
static struct platform_device zip_cl_rfkill = {
	.name = "zip_cl_rfkill",
	.id = -1,
};
#endif
...
static void __init register_i2c_devices(void)
{
	...
	msm8930_camera_i2c_devices.info = zip_cl_camera_board_info.board_info;
	msm8930_camera_i2c_devices.len = zip_cl_camera_board_info.num_i2c_board_info;
#endif

	/* Build the matching 'supported_machs' bitmask */
	if (machine_is_msm8930_cdp() || machine_is_msm8627_cdp() || machine_is_zip_cl())
		mach_mask = I2C_SURF;
...
static void __init zip_cl_init(void)
{
...
static void __init zip_cl_fixup(struct tag *tags,
...
MACHINE_START(ZIP_CL, "zip_cl")
	.fixup = zip_cl_fixup,
	.map_io = zip_cl_map_io,
	.reserve = zip_cl_reserve,
	.init_irq = zip_cl_init_irq,
	.handle_irq = gic_handle_irq,
	.timer = &msm_timer,
	.init_machine = zip_cl_init,
	.init_early = msm8930_allocate_memory_regions,
	.init_very_early = zip_cl_early_memory,
	.restart = msm_restart,
MACHINE_END
