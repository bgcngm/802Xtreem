...
struct pm8xxx_gpio_init dlxp_wl_bt_pmic_gpio[] = {
...
/* bt on configuration */
static uint32_t dlxp_wl_GPIO_bt_on_table[] = {
...
static uint32_t dlxp_wl_GPIO_bt_off_table[] = {
...
static void dlxp_wl_GPIO_config_bt_on(void)
{
	printk(KERN_INFO "[BT]== R ON ==\n");

	/* set bt on configuration*/
	config_bt_table(dlxp_wl_GPIO_bt_on_table,
				ARRAY_SIZE(dlxp_wl_GPIO_bt_on_table));
	mdelay(2);
...
static void dlxp_wl_GPIO_config_bt_off(void)
{
	...
	config_bt_table(dlxp_wl_GPIO_bt_off_table,
				ARRAY_SIZE(dlxp_wl_GPIO_bt_off_table));
	mdelay(2);
...
{
	if (!blocked)
		dlxp_wl_GPIO_config_bt_on();
	else
		dlxp_wl_GPIO_config_bt_off();

	return 0;
}

static struct rfkill_ops dlxp_wl_rfkill_ops = {
	.set_block = bluetooth_set_power,
};

static int dlxp_wl_rfkill_probe(struct platform_device *pdev)
{
	int rc = 0;
	...
	for( i = 0; i < ARRAY_SIZE(dlxp_wl_bt_pmic_gpio); i++) {
		rc = pm8xxx_gpio_config(dlxp_wl_bt_pmic_gpio[i].gpio,
					&dlxp_wl_bt_pmic_gpio[i].config);
		if (rc)
			pr_info("[bt] %s: Config ERROR: GPIO=%u, rc=%d\n",
				__func__, dlxp_wl_bt_pmic_gpio[i].gpio, rc);
	}
...
static int dlxp_wl_rfkill_remove(struct platform_device *dev)
{
...
static struct platform_driver dlxp_wl_rfkill_driver = {
	.probe = dlxp_wl_rfkill_probe,
	.remove = dlxp_wl_rfkill_remove,
	.driver = {
		.name = "dlxp_wl_rfkill",
		.owner = THIS_MODULE,
	},
...
static int __init dlxp_wl_rfkill_init(void)
{
	return platform_driver_register(&dlxp_wl_rfkill_driver);
}

static void __exit dlxp_wl_rfkill_exit(void)
{
	platform_driver_unregister(&dlxp_wl_rfkill_driver);
}

module_init(dlxp_wl_rfkill_init);
module_exit(dlxp_wl_rfkill_exit);
MODULE_DESCRIPTION("dlxp_wl rfkill");
