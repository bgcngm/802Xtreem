 ...
 *
 * HTC: jel_dd machine driver which defines board-specific data
...
#define PAMP_GPIO	(PM8921_GPIO_PM_TO_SYS(JEL_DD_PMGPIO_AUD_AMP_EN))
...
void jel_dd_set_q6_effect_mode(int mode)
{
	pr_aud_info("%s: mode %d\n", __func__, mode);
	atomic_set(&q6_effect_mode, mode);
}

int jel_dd_get_q6_effect_mode(void)
{
...
static struct acoustic_ops acoustic = {
	.set_q6_effect = jel_dd_set_q6_effect_mode,
};

static struct q6asm_ops qops = {
	.get_q6_effect = jel_dd_get_q6_effect_mode,
};
...
static int __init jel_dd_audio_init(void)
{
	int ret = 0, rc = 0;
	...
	rc = gpio_request(PM8921_GPIO_PM_TO_SYS(JEL_DD_PMGPIO_AUD_AMP_EN),
					"jel_dd_en");
	rc = pm8xxx_gpio_config(PM8921_GPIO_PM_TO_SYS(JEL_DD_PMGPIO_AUD_AMP_EN),
			&param);
	if (rc < 0)
...
late_initcall(jel_dd_audio_init);

static void __exit jel_dd_audio_exit(void)
{
