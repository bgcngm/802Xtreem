...
struct msm_gpiomux_config dlp_china_ethernet_configs[] = {
/*	{
		.gpio = 43,
		.settings = {
...
#endif
static struct msm_gpiomux_config dlp_china_hsic_configs[] = {
	{
		.gpio = 88,               /*HSIC_STROBE */
		.settings = {
...
static struct msm_gpiomux_config dlp_china_gsbi_configs[] __initdata = {
	{
...
static struct msm_gpiomux_config dlp_china_slimbus_config[] __initdata = {
	{
		.gpio   = 40,           /* slimbus clk */
		.settings = {
...
static struct msm_gpiomux_config dlp_china_audio_codec_configs[] __initdata = {
	{
		.gpio = 39,
		.settings = {
...
struct msm_gpiomux_config dlp_china_rotate_key_config[] = {
	{
		.gpio = 46,
		.settings = {
...
void __init dlp_china_init_gpiomux(void)
{
	int rc;
...
#if defined(CONFIG_KS8851) || defined(CONFIG_KS8851_MODULE)
	msm_gpiomux_install(dlp_china_ethernet_configs,
			ARRAY_SIZE(dlp_china_ethernet_configs));
#endif

	msm_gpiomux_install(wcnss_5wire_interface,
			ARRAY_SIZE(wcnss_5wire_interface));

	msm_gpiomux_install(dlp_china_gsbi_configs,
			ARRAY_SIZE(dlp_china_gsbi_configs));

	msm_gpiomux_install(dlp_china_slimbus_config,
			ARRAY_SIZE(dlp_china_slimbus_config));

	msm_gpiomux_install(dlp_china_audio_codec_configs,
			ARRAY_SIZE(dlp_china_audio_codec_configs));
...
#ifdef CONFIG_USB_EHCI_MSM_HSIC
	msm_gpiomux_install(dlp_china_hsic_configs,
		ARRAY_SIZE(dlp_china_hsic_configs));
#endif
