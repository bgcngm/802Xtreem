...
static void cp5_wl_amp_speaker(bool en)
{
...
static void cp5_wl_amp_hac(bool en)
{
}

struct htc_8930_gpio_pdata htc_audio_gpio = {
	.amp_speaker = cp5_wl_amp_speaker,
	.amp_receiver = cp5_wl_amp_receiver,
	.amp_headset = cp5_wl_amp_headset,
	.amp_hac = cp5_wl_amp_hac,
	.mi2s_gpio = {
	{
...
static int cp5_wl_get_hw_revision(void)
{
	int audio_hw_rev;
...
static int cp5_wl_get_hw_component(void)
{
	int hw_com = 0;
...
static int cp5_wl_enable_digital_mic(void)
{
	return 0;
}

static void cp5_wl_set_q6_effect_mode(int mode)
{
	pr_info("%s: mode %d\n", __func__, mode);
	atomic_set(&q6_effect_mode, mode);
}

static int cp5_wl_get_q6_effect_mode(void)
{
...
static int cp5_wl_get_24b_audio(void)
{
	return 1;
}

static struct acoustic_ops acoustic = {
	.set_q6_effect = cp5_wl_set_q6_effect_mode,
	.get_htc_revision = cp5_wl_get_hw_revision,
	.get_hw_component = cp5_wl_get_hw_component,
	.enable_digital_mic = cp5_wl_enable_digital_mic,
	.get_24b_audio = cp5_wl_get_24b_audio,
};

static struct q6asm_ops qops = {
	.get_q6_effect = cp5_wl_get_q6_effect_mode,
};
...
static int __init cp5_wl_audio_init(void)
{
	int ret = 0;
	pr_info("%s", __func__);
...
module_init(cp5_wl_audio_init);

static void __exit cp5_wl_audio_exit(void)
{
	pr_info("%s", __func__);
}
module_exit(cp5_wl_audio_exit);

MODULE_DESCRIPTION("ALSA Board CP5_WL");
