/* linux/arch/arm/mach-msm/board-cp5_wl-wifi.c
*/
...
#include "board-cp5_wl-wifi.h"

int cp5_wl_wifi_power(int on);
int cp5_wl_wifi_reset(int on);
int cp5_wl_wifi_set_carddetect(int on);
int cp5_wl_wifi_get_mac_addr(unsigned char *buf);
...
static void *cp5_wl_wifi_mem_prealloc(int section, unsigned long size)
{
...
int __init cp5_wl_init_wifi_mem(void)
{
	int i;
...
static struct resource cp5_wl_wifi_resources[] = {
	[0] = {
...
static struct wifi_platform_data cp5_wl_wifi_control = {
	.set_power      = cp5_wl_wifi_power,
	.set_reset      = cp5_wl_wifi_reset,
	.set_carddetect = cp5_wl_wifi_set_carddetect,
	.mem_prealloc   = cp5_wl_wifi_mem_prealloc,
	.get_mac_addr	= cp5_wl_wifi_get_mac_addr,
	.bus_scale_table    = &wlan_bus_scale_pdata,
};

static struct platform_device cp5_wl_wifi_device = {
	.name           = "bcmdhd_wlan",
	.id             = 1,
	.num_resources  = ARRAY_SIZE(cp5_wl_wifi_resources),
	.resource       = cp5_wl_wifi_resources,
	.dev            = {
		.platform_data = &cp5_wl_wifi_control,
	},
};

static unsigned cp5_wl_wifi_update_nvs(char *str)
{
...
#else
	cp5_wl_wifi_update_nvs("sd_oobonly=1\n");
#endif
	cp5_wl_wifi_update_nvs("btc_params80=0\n");
	cp5_wl_wifi_update_nvs("btc_params6=30\n");
	cp5_wl_init_wifi_mem();
	ret = platform_device_register(&cp5_wl_wifi_device);
	return ret;
}
