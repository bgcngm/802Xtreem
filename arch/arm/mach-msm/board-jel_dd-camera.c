...
static int jel_dd_csi_vreg_on(void);
static int jel_dd_csi_vreg_off(void);
...
struct msm_camera_device_platform_data msm_camera_csi_device_data[] = {
	{
		...
		.csid_core = 0,
		.camera_csi_on = jel_dd_csi_vreg_on,
		.camera_csi_off = jel_dd_csi_vreg_off,
...
static int jel_dd_use_ext_1v2(void)
{
	return 1;
}

#define JEL_DD_V_RAW_1V8_EN PM8921_GPIO_PM_TO_SYS(JEL_DD_PMGPIO_RAW_1V8_EN)
#define JEL_DD_V_RAW_1V2_EN PM8921_GPIO_PM_TO_SYS(JEL_DD_PMGPIO_RAW_1V2_EN)
static int jel_dd_rawchip_vreg_on(void)
{
	int rc;
	pr_info("%s\n", __func__);

	/* PM8921_GPIO_PM_TO_SYS(JEL_DD_PMGPIO_RAW_1V8_EN) 1800000 */
	rc = gpio_request(JEL_DD_V_RAW_1V8_EN, "V_RAW_1V8_EN");
	if (rc) {
		pr_err("[CAM] rawchip on\
			(\"gpio %d\", 1.2V) FAILED %d\n",
			JEL_DD_V_RAW_1V8_EN, rc);
		goto enable_1v8_fail;
	}
	gpio_direction_output(JEL_DD_V_RAW_1V8_EN, 1);
	gpio_free(JEL_DD_V_RAW_1V8_EN);

	mdelay(5);
	if (jel_dd_use_ext_1v2()) { /* use external 1v2 for HW workaround */
		rc = gpio_request(JEL_DD_GPIO_V_CAM_D1V2_EN, "rawchip");
		pr_info("rawchip external 1v2 gpio_request,%d\n",
		JEL_DD_GPIO_V_CAM_D1V2_EN);
		if (rc < 0) {
			pr_err("GPIO(%d) request failed", JEL_DD_GPIO_V_CAM_D1V2_EN);
			goto enable_1v2_fail;
		}
		gpio_direction_output(JEL_DD_GPIO_V_CAM_D1V2_EN, 1);
		gpio_free(JEL_DD_GPIO_V_CAM_D1V2_EN);
		mdelay(1);
	}
	return rc;
...
enable_1v2_fail:
	...
	if (rc)
		pr_err("[CAM] rawchip on\
			(\"gpio %d\", 1.2V) FAILED %d\n",
			JEL_DD_V_RAW_1V8_EN, rc);
	gpio_direction_output(JEL_DD_V_RAW_1V8_EN, 0);
	gpio_free(JEL_DD_V_RAW_1V8_EN);
enable_1v8_fail:
	return rc;
}

static int jel_dd_rawchip_vreg_off(void)
{
	int rc = 0;
	...
	if (jel_dd_use_ext_1v2()) { /* use external 1v2 for HW workaround */
		...
		if (rc)
			pr_err("[CAM] rawchip off(\
				\"gpio %d\", 1.2V) FAILED %d\n",
				JEL_DD_GPIO_V_CAM_D1V2_EN, rc);
		gpio_direction_output(JEL_DD_GPIO_V_CAM_D1V2_EN, 0);
		gpio_free(JEL_DD_GPIO_V_CAM_D1V2_EN);
...
static struct msm_camera_rawchip_info msm_rawchip_board_info = {
	.rawchip_reset	= JEL_DD_GPIO_RAW_RSTN,
	.rawchip_intr0	= MSM_GPIO_TO_INT(JEL_DD_GPIO_RAW_INTR0),
	.rawchip_intr1	= MSM_GPIO_TO_INT(JEL_DD_GPIO_RAW_INTR1),
	...
	.camera_rawchip_power_on = jel_dd_rawchip_vreg_on,
	.camera_rawchip_power_off = jel_dd_rawchip_vreg_off,
	.rawchip_use_ext_1v2 = jel_dd_use_ext_1v2,
};
...
static uint16_t msm_cam_gpio_tbl[] = {
	JEL_DD_GPIO_CAM_MCLK0, /*CAMIF_MCLK*/
	JEL_DD_GPIO_CAM_MCLK1,
	JEL_DD_GPIO_RAW_INTR0,
	JEL_DD_GPIO_RAW_INTR1,
	JEL_DD_GPIO_MCAM_SPI_CLK,
	JEL_DD_GPIO_MCAM_SPI_CS0,
	JEL_DD_GPIO_MCAM_SPI_DI,
	JEL_DD_GPIO_MCAM_SPI_DO,
};
...
static int jel_dd_s5k3h2yx_vreg_on(void)
{
	int rc;
	pr_info("%s\n", __func__);

	/* VCM */
	if (jel_dd_use_ext_1v2())
	...
	/* digital */
	rc = gpio_request(JEL_DD_GPIO_V_CAM_D1V2_EN, "CAM_D1V2_EN");
	if (rc) {
		pr_err("sensor_power_enable\
			(\"gpio %d\", 1.2V) FAILED %d\n",
			JEL_DD_GPIO_V_CAM_D1V2_EN, rc);
...
static int jel_dd_s5k3h2yx_vreg_off(void)
{
	int rc = 0;
	...
	/* VCM */
	if (jel_dd_use_ext_1v2() == 0)/*XA only*/
...
static struct msm_actuator_info s5k3h2yx_actuator_info = {
	...
	.vcm_pwd        = JEL_DD_GPIO_CAM_VCM_PD,
	.vcm_enable     = 1,
};
#endif
...
static struct msm_camera_sensor_platform_info sensor_s5k3h2yx_board_info = {
	...
	.sensor_reset	= 0,
	.sensor_pwd	= JEL_DD_GPIO_CAM_PWDN,
	.vcm_pwd	= JEL_DD_GPIO_CAM_VCM_PD,
	.vcm_enable	= 1,
...
static struct msm_camera_sensor_info msm_camera_sensor_s5k3h2yx_data = {
	...
	.camera_power_on = jel_dd_s5k3h2yx_vreg_on,
	.camera_power_off = jel_dd_s5k3h2yx_vreg_off,
...
static int jel_dd_s5k6a1gx_vreg_on(void)
{
	int rc;
	pr_info("[CAM] %s\n", __func__);
	...
	if (jel_dd_use_ext_1v2()) { /* use external 1v2 for HW workaround */
		...
		rc = gpio_request(JEL_DD_GPIO_V_CAM2_D1V8_EN, "s5k6a1gx");
		pr_info("[CAM]power IO gpio_request,%d\n", JEL_DD_GPIO_V_CAM2_D1V8_EN);
		if (rc < 0) {
			pr_err("GPIO(%d) request failed", JEL_DD_GPIO_V_CAM2_D1V8_EN);
			goto enable_io_fail;
		}
		gpio_direction_output(JEL_DD_GPIO_V_CAM2_D1V8_EN, 1);
		gpio_free(JEL_DD_GPIO_V_CAM2_D1V8_EN);
		udelay(50);
	...
	udelay(50);
	/* reset pin */
	rc = gpio_request(JEL_DD_GPIO_CAM2_RSTz, "s5k6a1gx");
	pr_info("[CAM]reset pin gpio_request,%d\n", JEL_DD_GPIO_CAM2_RSTz);
	if (rc < 0) {
		pr_err("GPIO(%d) request failed", JEL_DD_GPIO_CAM2_RSTz);
		goto enable_rst_fail;
	}
	gpio_direction_output(JEL_DD_GPIO_CAM2_RSTz, 1);
	gpio_free(JEL_DD_GPIO_CAM2_RSTz);
	udelay(50);
	...
	if (jel_dd_use_ext_1v2()) { /* use external 1v2 for HW workaround */
		...
		rc = gpio_request(JEL_DD_GPIO_V_CAM_D1V2_EN, "s5k6a1gx");
		pr_info("[CAM]digital gpio_request,%d\n", JEL_DD_GPIO_V_CAM_D1V2_EN);
		if (rc < 0) {
			pr_err("GPIO(%d) request failed", JEL_DD_GPIO_V_CAM_D1V2_EN);
			goto enable_digital_fail;
...
enable_digital_fail:
	...
	if (rc < 0)
		...
		gpio_direction_output(JEL_DD_GPIO_CAM2_RSTz, 0);
		gpio_free(JEL_DD_GPIO_CAM2_RSTz);
	}
enable_rst_fail:
...
enable_i2c_pullup_fail:
	...
	if (rc < 0)
		...
		gpio_direction_output(JEL_DD_GPIO_V_CAM2_D1V8_EN, 0);
		gpio_free(JEL_DD_GPIO_V_CAM2_D1V8_EN);
	}
enable_io_fail:
	camera_sensor_power_disable(reg_8921_l8);
...
static int jel_dd_s5k6a1gx_vreg_off(void)
{
	int rc;
	pr_info("[CAM] %s\n", __func__);
...
static struct msm_camera_sensor_platform_info sensor_s5k6a1gx_board_info = {
	...
	.sensor_reset_enable = 0,
	.sensor_reset	= JEL_DD_GPIO_CAM2_RSTz,
	.vcm_pwd	= 0,
	.vcm_enable	= 0,
...
static struct msm_camera_sensor_info msm_camera_sensor_s5k6a1gx_data = {
	...
	.vcm_pwd	= 0,
	.vcm_enable	= 0,
	.camera_power_on = jel_dd_s5k6a1gx_vreg_on,
	.camera_power_off = jel_dd_s5k6a1gx_vreg_off,
...
struct msm_camera_board_info jel_dd_camera_board_info = {
