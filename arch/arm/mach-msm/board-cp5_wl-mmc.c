/* linux/arch/arm/mach-msm/board-cp5_wl-mmc.c
 *
 ...
 */
static struct embedded_sdio_data cp5_wl_wifi_emb_data = {
	.cccr	= {
		.sdio_vsn	= 2,
		.multi_block	= 1,
		.low_speed	= 0,
...
static int
cp5_wl_wifi_status_register(void (*callback)(int card_present, void *dev_id),
				void *dev_id)
{
	if (wifi_status_cb)
		return -EAGAIN;
...
static int cp5_wl_wifi_cd;	/* WiFi virtual 'card detect' status */

static unsigned int cp5_wl_wifi_status(struct device *dev)
{
	return cp5_wl_wifi_cd;
}

static unsigned int cp5_wl_wifislot_type = MMC_TYPE_SDIO_WIFI;
static unsigned int wifi_sup_clk_rates[] = {
	400000, 24000000, 48000000,
};
static struct mmc_platform_data cp5_wl_wifi_data = {
	.ocr_mask               = MMC_VDD_28_29,
	.status                 = cp5_wl_wifi_status,
	.register_status_notify = cp5_wl_wifi_status_register,
	.embedded_sdio          = &cp5_wl_wifi_emb_data,
	.mmc_bus_width  = MMC_CAP_4_BIT_DATA,
	.sup_clk_table	= wifi_sup_clk_rates,
	.sup_clk_cnt	= ARRAY_SIZE(wifi_sup_clk_rates),
	.slot_type = &cp5_wl_wifislot_type,
...
int cp5_wl_wifi_set_carddetect(int val)
{
...
EXPORT_SYMBOL(cp5_wl_wifi_set_carddetect);
...
EXPORT_SYMBOL(cp5_wl_wifi_power);
...
/* ---- MMC ---- */
int __init cp5_wl_init_mmc()
{
	uint32_t id;
	struct pm_gpio pm_config;
	wifi_status_cb = NULL;

	printk(KERN_INFO "cp5_wl: %s\n", __func__);
	...
	/* PM QoS for wifi*/
	cp5_wl_wifi_data.cpu_dma_latency = msm_rpmrs_levels[0].latency_us;

	msm_add_sdcc(4, &cp5_wl_wifi_data);
