#if 0
/* linux/arch/arm/mach-msm/m7china-wifi.c
*/
...
#include "board-m7china-wifi.h"

int m7china_wifi_power(int on);
int m7china_wifi_reset(int on);
int m7china_wifi_set_carddetect(int on);
int m7china_wifi_get_mac_addr(unsigned char *buf);
...
static void *m7china_wifi_mem_prealloc(int section, unsigned long size)
{
...
int __init m7china_init_wifi_mem(void)
{
	int i;
...
static struct resource m7china_wifi_resources[] = {
	[0] = {
...
static struct wifi_platform_data m7china_wifi_control = {
	.set_power      = m7china_wifi_power,
	.set_reset      = m7china_wifi_reset,
	.set_carddetect = m7china_wifi_set_carddetect,
	.mem_prealloc   = m7china_wifi_mem_prealloc,
	.get_mac_addr	= m7china_wifi_get_mac_addr,
};

static struct platform_device m7china_wifi_device = {
	.name           = "bcmdhd_wlan",
	.id             = 1,
	.num_resources  = ARRAY_SIZE(m7china_wifi_resources),
	.resource       = m7china_wifi_resources,
	.dev            = {
		.platform_data = &m7china_wifi_control,
	},
};

static unsigned m7china_wifi_update_nvs(char *str)
{
...
#else
	m7china_wifi_update_nvs("sd_oobonly=1\n");
#endif
	m7china_wifi_update_nvs("btc_params80=0\n");
	m7china_wifi_update_nvs("btc_params6=30\n");
	m7china_init_wifi_mem();
	ret = platform_device_register(&m7china_wifi_device);
	return ret;
}
#endif
