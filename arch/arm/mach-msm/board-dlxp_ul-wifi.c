/* linux/arch/arm/mach-msm/dlxp_ul-wifi.c
*/
...
#include "board-dlxp_ul-wifi.h"

int dlxp_ul_wifi_power(int on);
int dlxp_ul_wifi_reset(int on);
int dlxp_ul_wifi_set_carddetect(int on);
int dlxp_ul_wifi_get_mac_addr(unsigned char *buf);
...
static void *dlxp_ul_wifi_mem_prealloc(int section, unsigned long size)
{
...
int __init dlxp_ul_init_wifi_mem(void)
{
	int i;
...
static struct resource dlxp_ul_wifi_resources[] = {
	[0] = {
...
static struct wifi_platform_data dlxp_ul_wifi_control = {
	.set_power      = dlxp_ul_wifi_power,
	.set_reset      = dlxp_ul_wifi_reset,
	.set_carddetect = dlxp_ul_wifi_set_carddetect,
	.mem_prealloc   = dlxp_ul_wifi_mem_prealloc,
	.get_mac_addr	= dlxp_ul_wifi_get_mac_addr,
};

static struct platform_device dlxp_ul_wifi_device = {
	.name           = "bcmdhd_wlan",
	.id             = 1,
	.num_resources  = ARRAY_SIZE(dlxp_ul_wifi_resources),
	.resource       = dlxp_ul_wifi_resources,
	.dev            = {
		.platform_data = &dlxp_ul_wifi_control,
	},
};

static unsigned dlxp_ul_wifi_update_nvs(char *str)
{
...
#else
	dlxp_ul_wifi_update_nvs("sd_oobonly=1\n");
#endif
	dlxp_ul_wifi_update_nvs("btc_params80=0\n");
	dlxp_ul_wifi_update_nvs("btc_params6=30\n");
	dlxp_ul_init_wifi_mem();
	ret = platform_device_register(&dlxp_ul_wifi_device);
	return ret;
}
